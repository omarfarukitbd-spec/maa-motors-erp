const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/dao-DAmjbxH1.js","assets/rolldown-runtime-Dd_uD5pT.js","assets/vendor-firebase-YQIUDKRL.js","assets/vendor-CJahiyzm.js","assets/vendor-CwbMEznW.css"])))=>i.map(i=>d[i]);
import{i as e}from"./rolldown-runtime-Dd_uD5pT.js";import{s as t}from"./dao-DAmjbxH1.js";import{n}from"./vendor-ui-n4g2UPZQ.js";import{a as r,d as i,i as a,l as o,n as s,o as c,s as l,u}from"./index-Dxnn8_PK.js";var d=e(n());function f(e,t){let n=[],r=e,i=`<tr style="background:#f1f5f9; border-bottom:2px solid #cbd5e1;">
        ...
        <td style="text-align:right; font-weight:900; color:#0f172a; padding: 8px 12px; border-left:1px solid #cbd5e1; background:#fff;">
            ৳ ${o(Math.abs(e))} ${e<0?`(Adv)`:``}
        </td>
    </tr>`;return n.push({html:i,textLength:15}),t.forEach(e=>{let t=Number(e.bill)||0,i=Number(e.paid)||0;e.receivedType,r=u(r+(t-i));let a=``;if(e.createdAt)try{let t=e.createdAt.toDate?e.createdAt.toDate():e.createdAt.toMillis?new Date(e.createdAt.toMillis()):new Date(e.createdAt);isNaN(t.getTime())||(a=t.toLocaleTimeString(`en-US`,{hour:`numeric`,minute:`2-digit`,hour12:!0}))}catch(e){console.error(`Time parsing error in statement print:`,e)}let s=`-`;if(i>0){let t=e.receivedType||`Bank`,n=(e.receivedFrom||``).trim(),r=n?`${t}: ${n}`:t;s=t===`Less`?`<strong style="color:#7c3aed; font-size:10px; background:#f5f3ff; border:1px solid #ddd6fe; padding:1px 6px; border-radius:5px; display:inline-block;">[LESS] ${n||``}</strong>`:t===`Bank`?`<strong style="color:#0284c7; font-size:10px; background:#f0f9ff; border:1px solid #bae6fd; padding:1px 6px; border-radius:5px; display:inline-block;">${r}</strong>`:`<strong style="color:#059669; font-size:10px; background:#ecfdf5; border:1px solid #a7f3d0; padding:1px 6px; border-radius:5px; display:inline-block;">${r}</strong>`}else t>0&&e.notes&&(s=`<span style="font-size:10px; color:#475569;">${e.notes}</span>`);let l=e.voucherNo&&e.voucherNo!==`OPENING`?`<span style="font-size:9.5px; color:#0284c7; font-weight:900; font-family:monospace; margin-left:4px;">#${e.voucherNo}</span>`:``,d=`<tr>
            <td style="font-size:10.5px; border-bottom:1px solid #e2e8f0; padding: 5px 8px; color:#0f172a; line-height: 1.2; vertical-align: middle;">
                ...
                ${a?`<div style="font-size: 8px; color: #64748b; font-weight: 500; margin-top: 1px;">${a}</div>`:``}
            </td>
            <td style="font-size:11px; border-bottom:1px solid #e2e8f0; padding: 5px 10px; color:#0f172a; vertical-align: middle;">${s}${l}</td>
            <td style="text-align:right; color:#dc2626; font-weight:700; border-bottom:1px solid #e2e8f0; padding: 5px 10px; vertical-align: middle;">${t>0?o(t):`-`}</td>
            <td style="text-align:right; color:#059669; font-weight:700; border-bottom:1px solid #e2e8f0; padding: 5px 10px; vertical-align: middle;">${i>0?o(i):`-`}</td>
            <td style="text-align:right; font-weight:900; color:#0f172a; border-bottom:1px solid #e2e8f0; padding: 5px 10px; border-left:1px solid #e2e8f0; vertical-align: middle;">
                ৳ ${o(Math.abs(r))} ${r<0?`<span style="font-size:8px; color:#059669;">(Adv)</span>`:``}
            </td>
    ...
    `}}async function m(e,n,r,i=``){try{let o=await t.getAppSettings(),u=document.getElementById(`print-receipt-container`);u||(u=document.createElement(`div`),u.id=`print-receipt-container`,document.body.appendChild(u));let d=document.getElementById(`stmt-start-date`)?.value||``,m=document.getElementById(`stmt-end-date`)?.value||``,h=d||m?`${d?c(d):`শুরু`} হতে ${m?c(m):`আজ`}`:`সকল লেনদেন`,g=document.getElementById(`stmt-total-bill`)?.innerText||`৳ 0`,_=document.getElementById(`stmt-total-paid`)?.innerText||`৳ 0`,v=document.getElementById(`stmt-total-due`)?.innerText||`৳ 0`,{rowsArray:y,running:b}=f(n,r),{page1HeaderHtml:x,repeatHeaderHtml:S,page1ExtraHtml:C,tableColHeaderHtml:w,signatureHtml:T}=p(e,g,_,v,b,o,`CUSTOMER KHATIYAN`,`কাস্টমার বকেয়া খতিয়ান`,h),E=i&&i.trim()?`
            <div style="margin-top: 15px; padding: 10px 14px; background: #fffbe6; border: 1px solid #ffe58f; border-radius: 8px; font-size: 11px; color: #856404; font-family: sans-serif; page-break-inside: avoid; break-inside: avoid;">
                ...
                ${i.replace(/\n/g,`<br>`)}
            </div>`:``,[D,O,k]=l().split(`-`),A=await a({rowsArray:y,page1HeaderHtml:x,repeatHeaderHtml:S,tableColHeaderHtml:w,page1ExtraHtml:C,summaryHtml:E,signatureHtml:T,formattedDate:`${k}/${O}/${D}`});s(A)}catch(e){console.error(`Statement print error:`,e),d.default.fire(`Error`,`প্রিন্ট করতে সমস্যা হয়েছে`,`error`)}}var h=null;async function g(e){if(!e)return;let t=document.getElementById(`login-screen`),n=document.getElementById(`app-container`);t&&(t.style.display=`none`),n&&n.classList.add(`hidden`);let r=document.getElementById(`public-stmt-view`);r||(r=document.createElement(`div`),r.id=`public-stmt-view`,r.className=`fixed inset-0 z-[9999] overflow-y-auto bg-slate-950 p-3 sm:p-6 font-bn flex flex-col items-center justify-start`,document.body.appendChild(r)),r.innerHTML=`<div class="text-center py-20 text-white font-bold"><i class="fa-solid fa-spinner fa-spin text-2xl text-blue-500 mb-3"></i><p>মেসার্স মা মোটরস্ বিবরণী লোড হচ্ছে...</p></div>`;try{let{CustomerDAO:t,TransactionDAO:n,SettingsDAO:a}=await i(async()=>{let{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}=await import(`./dao-DAmjbxH1.js`).then(e=>e.d);return{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}},__vite__mapDeps([0,1,2,3,4])),s=await t.getById(e);if(!s)return r.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">কাস্টমার হিসাব পাওয়া যায়নি!</div>`;let c=await a.getAppSettings(),l=(await n.getByCustomer(e)).filter(e=>{let t=String(e.voucherNo||``).trim().toUpperCase();return t!==`OPENING`&&t!==`OPEN`&&t!==`প্রারম্ভিক ব্যালেন্স`&&t!==`প্রারম্ভিক জের`});l.sort((e,t)=>{let n=new Date(e.date)-new Date(t.date);return n===0?(e.createdAt?.toMillis()||0)-(t.createdAt?.toMillis()||0):n});let d=Number(s.initialDue||0),m=0,g=0,_=0;l.forEach(e=>{e.receivedType===`Less`?_=u(_+(Number(e.paid)||0)):g=u(g+(Number(e.paid)||0)),m=u(m+(Number(e.bill)||0))});let v=u(d+m-g-_),{rowsArray:y}=f(d,l);h={customer:s,docs:l,settings:c,initialDue:d,billSum:m,paidSum:g,lessSum:_,running:v};let{page1HeaderHtml:b,page1ExtraHtml:x}=p(s,`৳ ${o(m)}`,`৳ ${o(g)}`,`৳ ${o(Math.abs(v))} ${v<0?`(Adv)`:``}`,v,c,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`);r.innerHTML=`
            ...
            </div>`}catch(e){console.error(e),r.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">স্টেটমেন্ট লোড করতে ব্যর্থ!</div>`}}window.printPublicStatement=async()=>{if(!h)return;let{customer:e,docs:t,settings:n,initialDue:r,billSum:i,paidSum:c,running:u}=h,d=document.getElementById(`print-receipt-container`);d||(d=document.createElement(`div`),d.id=`print-receipt-container`,document.body.appendChild(d));let{rowsArray:m}=f(r,t),{page1HeaderHtml:g,repeatHeaderHtml:_,page1ExtraHtml:v,tableColHeaderHtml:y,signatureHtml:b}=p(e,`৳ ${o(i)}`,`৳ ${o(c)}`,`৳ ${o(Math.abs(u))} ${u<0?`(Adv)`:``}`,u,n,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`),[x,S,C]=l().split(`-`),w=await a({rowsArray:m,page1HeaderHtml:g,repeatHeaderHtml:_,tableColHeaderHtml:y,page1ExtraHtml:v,summaryHtml:``,signatureHtml:b,formattedDate:`${C}/${S}/${x}`});s(w)},window.renderPublicStatementView=g;export{m as printStatement,g as renderPublicStatementView};
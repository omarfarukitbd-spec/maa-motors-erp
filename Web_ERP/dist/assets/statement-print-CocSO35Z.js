...
import{i as e}from"./rolldown-runtime-Dd_uD5pT.js";import{r as t}from"./vendor-COdl40C-.js";import{s as n}from"./dao-DWBGBcb5.js";import{_ as r,c as i,m as a,s as o,u as s}from"./ui-helpers-DFfGYhR-.js";import{n as c}from"./vendor-ui-n4g2UPZQ.js";import{r as l,t as u}from"./index-B1WUyDaP.js";var d=e(c());function f(e,t){let n=[],o=e,s=`<tr style="background:#f1f5f9; border-bottom:2px solid #cbd5e1;">
        ...
        <td style="text-align:right; font-weight:900; color:#0f172a; padding: 8px 12px; border-left:1px solid #cbd5e1; background:#fff;">
            ...
            <td style="font-size:10.5px; border-bottom:1px solid #e2e8f0; padding: 5px 8px; color:#0f172a; line-height: 1.2; vertical-align: middle;">
                <div style="font-weight: 700;">${i(e.date)}</div>
            ...
            <td style="text-align:right; color:#059669; font-weight:700; border-bottom:1px solid #e2e8f0; padding: 5px 10px; vertical-align: middle;">${s>0?a(s):`-`}</td>
            <td style="text-align:right; font-weight:900; color:#0f172a; border-bottom:1px solid #e2e8f0; padding: 5px 10px; border-left:1px solid #e2e8f0; vertical-align: middle;">
                ৳ ${a(Math.abs(o))} ${o<0?`<span style="font-size:8px; color:#059669;">(Adv)</span>`:``}
            </td>
        </tr>`;n.push({html:d,textLength:(e.receivedFrom||``).length})}),{rowsArray:n,running:o}}function p(e,t,n,r,i,a,s,c,l){let u=(e.name||``).replace(/^\[.*?\]\s*/,``).trim();return{page1HeaderHtml:o(a,{title:s,subtitle:c,dateRangeStr:l}),repeatHeaderHtml:`
    ...
    `}}async function m(e,t,r,a=``){try{let o=await n.getAppSettings(),c=document.getElementById(`print-receipt-container`);c||(c=document.createElement(`div`),c.id=`print-receipt-container`,document.body.appendChild(c));let d=document.getElementById(`stmt-start-date`)?.value||``,m=document.getElementById(`stmt-end-date`)?.value||``,h=d||m?`${d?i(d):`শুরু`} হতে ${m?i(m):`আজ`}`:`সকল লেনদেন`,g=document.getElementById(`stmt-total-bill`)?.innerText||`৳ 0`,_=document.getElementById(`stmt-total-paid`)?.innerText||`৳ 0`,v=document.getElementById(`stmt-total-due`)?.innerText||`৳ 0`,{rowsArray:y,running:b}=f(t,r),{page1HeaderHtml:x,repeatHeaderHtml:S,page1ExtraHtml:C,tableColHeaderHtml:w,signatureHtml:T}=p(e,g,_,v,b,o,`CUSTOMER KHATIYAN`,`কাস্টমার বকেয়া খতিয়ান`,h),E=a&&a.trim()?`
            <div style="margin-top: 15px; padding: 10px 14px; background: #fffbe6; border: 1px solid #ffe58f; border-radius: 8px; font-size: 11px; color: #856404; font-family: sans-serif; page-break-inside: avoid; break-inside: avoid;">
                ...
                ${a.replace(/\n/g,`<br>`)}
            </div>`:``,[D,O,k]=s().split(`-`),A=await l({rowsArray:y,page1HeaderHtml:x,repeatHeaderHtml:S,tableColHeaderHtml:w,page1ExtraHtml:C,summaryHtml:E,signatureHtml:T,formattedDate:`${k}/${O}/${D}`});u(A)}catch(e){console.error(`Statement print error:`,e),d.default.fire(`Error`,`প্রিন্ট করতে সমস্যা হয়েছে`,`error`)}}var h=null;async function g(e){if(!e)return;let n=document.getElementById(`login-screen`),i=document.getElementById(`app-container`);n&&(n.style.display=`none`),i&&i.classList.add(`hidden`);let o=document.getElementById(`public-stmt-view`);o||(o=document.createElement(`div`),o.id=`public-stmt-view`,o.className=`fixed inset-0 z-[9999] overflow-y-auto bg-slate-950 p-3 sm:p-6 font-bn flex flex-col items-center justify-start`,document.body.appendChild(o)),o.innerHTML=`<div class="text-center py-20 text-white font-bold"><i class="fa-solid fa-spinner fa-spin text-2xl text-blue-500 mb-3"></i><p>মেসার্স মা মোটরস্ বিবরণী লোড হচ্ছে...</p></div>`;try{let{CustomerDAO:n,TransactionDAO:i,SettingsDAO:s}=await t(async()=>{let{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}=await import(`./dao-DWBGBcb5.js`).then(e=>e.d);return{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}},__vite__mapDeps([0,1,2,3,4])),c=await n.getById(e);if(!c)return o.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">কাস্টমার হিসাব পাওয়া যায়নি!</div>`;let l=await s.getAppSettings(),u=(await i.getByCustomer(e)).filter(e=>{let t=String(e.voucherNo||``).trim().toUpperCase();return t!==`OPENING`&&t!==`OPEN`&&t!==`প্রারম্ভিক ব্যালেন্স`&&t!==`প্রারম্ভিক জের`});u.sort((e,t)=>{let n=new Date(e.date)-new Date(t.date);return n===0?(e.createdAt?.toMillis()||0)-(t.createdAt?.toMillis()||0):n});let d=Number(c.initialDue||0),m=0,g=0,_=0;u.forEach(e=>{e.receivedType===`Less`?_=r(_+(Number(e.paid)||0)):g=r(g+(Number(e.paid)||0)),m=r(m+(Number(e.bill)||0))});let v=r(d+m-g-_),{rowsArray:y}=f(d,u);h={customer:c,docs:u,settings:l,initialDue:d,billSum:m,paidSum:g,lessSum:_,running:v};let{page1HeaderHtml:b,page1ExtraHtml:x}=p(c,`৳ ${a(m)}`,`৳ ${a(g)}`,`৳ ${a(Math.abs(v))} ${v<0?`(Adv)`:``}`,v,l,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`);o.innerHTML=`
            ...
            </div>`}catch(e){console.error(e),o.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">স্টেটমেন্ট লোড করতে ব্যর্থ!</div>`}}window.printPublicStatement=async()=>{if(!h)return;let{customer:e,docs:t,settings:n,initialDue:r,billSum:i,paidSum:o,running:c}=h,d=document.getElementById(`print-receipt-container`);d||(d=document.createElement(`div`),d.id=`print-receipt-container`,document.body.appendChild(d));let{rowsArray:m}=f(r,t),{page1HeaderHtml:g,repeatHeaderHtml:_,page1ExtraHtml:v,tableColHeaderHtml:y,signatureHtml:b}=p(e,`৳ ${a(i)}`,`৳ ${a(o)}`,`৳ ${a(Math.abs(c))} ${c<0?`(Adv)`:``}`,c,n,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`),[x,S,C]=s().split(`-`),w=await l({rowsArray:m,page1HeaderHtml:g,repeatHeaderHtml:_,tableColHeaderHtml:y,page1ExtraHtml:v,summaryHtml:``,signatureHtml:b,formattedDate:`${C}/${S}/${x}`});u(w)},window.renderPublicStatementView=g;export{m as printStatement,g as renderPublicStatementView};
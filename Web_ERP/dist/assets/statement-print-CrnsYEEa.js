...
import{i as e}from"./rolldown-runtime-Dd_uD5pT.js";import{s as t}from"./dao-CJk3sbx1.js";import{a as n}from"./audit-BuamRIjn.js";import{_ as r,b as i,c as a,f as o,l as s,p as c}from"./ui-helpers-vB-cDBaI.js";import{n as l}from"./vendor-ui-n4g2UPZQ.js";import{r as u,t as d}from"./index-UFICEZQ3.js";var f=e(l());function p(e,t){let n=[],a=e,c=`<tr style="background:#f1f5f9; border-bottom:2px solid #cbd5e1;">
        ...
        </tr>`;n.push({html:f,textLength:(e.receivedFrom||``).length})}),{rowsArray:n,running:a}}function m(e,t,n,r,i,o,s,c,l){let u=(e.name||``).replace(/^\[.*?\]\s*/,``).trim();return{page1HeaderHtml:a(o,{title:s,subtitle:c,dateRangeStr:l}),repeatHeaderHtml:`
    ...
    `}}async function h(e,n,r,i=``){try{let a=await t.getAppSettings(),o=document.getElementById(`print-receipt-container`);o||(o=document.createElement(`div`),o.id=`print-receipt-container`,document.body.appendChild(o));let l=document.getElementById(`stmt-start-date`)?.value||``,f=document.getElementById(`stmt-end-date`)?.value||``,h=l||f?`${l?s(l):`শুরু`} হতে ${f?s(f):`আজ`}`:`সকল লেনদেন`,g=document.getElementById(`stmt-total-bill`)?.innerText||`৳ 0`,_=document.getElementById(`stmt-total-paid`)?.innerText||`৳ 0`,v=document.getElementById(`stmt-total-due`)?.innerText||`৳ 0`,{rowsArray:y,running:b}=p(n,r),{page1HeaderHtml:x,repeatHeaderHtml:S,page1ExtraHtml:C,tableColHeaderHtml:w,signatureHtml:T}=m(e,g,_,v,b,a,`CUSTOMER KHATIYAN`,`কাস্টমার বকেয়া খতিয়ান`,h),E=i&&i.trim()?`
            <div style="margin-top: 15px; padding: 10px 14px; background: #fffbe6; border: 1px solid #ffe58f; border-radius: 8px; font-size: 11px; color: #856404; font-family: sans-serif; page-break-inside: avoid; break-inside: avoid;">
                ...
                ${i.replace(/\n/g,`<br>`)}
            </div>`:``,[D,O,k]=c().split(`-`),A=await u({rowsArray:y,page1HeaderHtml:x,repeatHeaderHtml:S,tableColHeaderHtml:w,page1ExtraHtml:C,summaryHtml:E,signatureHtml:T,formattedDate:`${k}/${O}/${D}`});d(A)}catch(e){console.error(`Statement print error:`,e),f.default.fire(`Error`,`প্রিন্ট করতে সমস্যা হয়েছে`,`error`)}}var g=null;async function _(e){if(!e)return;let t=document.getElementById(`login-screen`),a=document.getElementById(`app-container`);t&&(t.style.display=`none`),a&&a.classList.add(`hidden`);let o=document.getElementById(`public-stmt-view`);o||(o=document.createElement(`div`),o.id=`public-stmt-view`,o.className=`fixed inset-0 z-[9999] overflow-y-auto bg-slate-950 p-3 sm:p-6 font-bn flex flex-col items-center justify-start`,document.body.appendChild(o)),o.innerHTML=`<div class="text-center py-20 text-white font-bold"><i class="fa-solid fa-spinner fa-spin text-2xl text-blue-500 mb-3"></i><p>মেসার্স মা মোটরস্ বিবরণী লোড হচ্ছে...</p></div>`;try{let{CustomerDAO:t,TransactionDAO:a,SettingsDAO:s}=await n(async()=>{let{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}=await import(`./dao-CJk3sbx1.js`).then(e=>e.d);return{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}},__vite__mapDeps([0,1,2,3,4])),c=await t.getById(e);if(!c)return o.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">কাস্টমার হিসাব পাওয়া যায়নি!</div>`;let l=await s.getAppSettings(),u=(await a.getByCustomer(e)).filter(e=>{let t=String(e.voucherNo||``).trim().toUpperCase();return t!==`OPENING`&&t!==`OPEN`&&t!==`প্রারম্ভিক ব্যালেন্স`&&t!==`প্রারম্ভিক জের`});u.sort((e,t)=>{let n=new Date(e.date)-new Date(t.date);return n===0?(e.createdAt?.toMillis()||0)-(t.createdAt?.toMillis()||0):n});let d=Number(c.initialDue||0),f=0,h=0,_=0;u.forEach(e=>{e.receivedType===`Less`?_=i(_+(Number(e.paid)||0)):h=i(h+(Number(e.paid)||0)),f=i(f+(Number(e.bill)||0))});let v=i(d+f-h-_),{rowsArray:y}=p(d,u);g={customer:c,docs:u,settings:l,initialDue:d,billSum:f,paidSum:h,lessSum:_,running:v};let{page1HeaderHtml:b,page1ExtraHtml:x}=m(c,`৳ ${r(f)}`,`৳ ${r(h)}`,`৳ ${r(Math.abs(v))} ${v<0?`(Adv)`:``}`,v,l,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`);o.innerHTML=`
            ...
            </div>`}catch(e){console.error(e),o.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">স্টেটমেন্ট লোড করতে ব্যর্থ!</div>`}}window.printPublicStatement=async()=>{if(!g)return;let{customer:e,docs:t,settings:n,initialDue:i,billSum:a,paidSum:o,running:s}=g,l=document.getElementById(`print-receipt-container`);l||(l=document.createElement(`div`),l.id=`print-receipt-container`,document.body.appendChild(l));let{rowsArray:f}=p(i,t),{page1HeaderHtml:h,repeatHeaderHtml:_,page1ExtraHtml:v,tableColHeaderHtml:y,signatureHtml:b}=m(e,`৳ ${r(a)}`,`৳ ${r(o)}`,`৳ ${r(Math.abs(s))} ${s<0?`(Adv)`:``}`,s,n,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`),[x,S,C]=c().split(`-`),w=await u({rowsArray:f,page1HeaderHtml:h,repeatHeaderHtml:_,tableColHeaderHtml:y,page1ExtraHtml:v,summaryHtml:``,signatureHtml:b,formattedDate:`${C}/${S}/${x}`});d(w)},window.renderPublicStatementView=_;export{h as printStatement,_ as renderPublicStatementView};
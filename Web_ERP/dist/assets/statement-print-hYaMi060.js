const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/dao-BJg8z6Lo.js","assets/rolldown-runtime-Dd_uD5pT.js","assets/vendor-firebase-YQIUDKRL.js","assets/vendor-CJahiyzm.js","assets/vendor-CwbMEznW.css"])))=>i.map(i=>d[i]);
import{i as e}from"./rolldown-runtime-Dd_uD5pT.js";import{o as t}from"./dao-BJg8z6Lo.js";import{O as n}from"./customer-state-B1yFrRvu.js";import{c as r,d as i,o as a,p as o,s}from"./ui-helpers-IvKNSVag.js";import{n as c}from"./vendor-ui-n4g2UPZQ.js";import{r as l,t as u}from"./smart-print-engine-B0360BA8.js";var d=e(c());function f(e,t){let n=[],r=e,a=`<tr style="background:#f1f5f9; border-bottom:2px solid #cbd5e1;">
        ...
        </tr>`;n.push({html:d,textLength:(e.receivedFrom||``).length})}),{rowsArray:n,running:r}}function p(e,t,n,r,i,o,s,c,l){let u=(e.name||``).replace(/^\[.*?\]\s*/,``).trim();return{page1HeaderHtml:a(o,{title:s,subtitle:c,dateRangeStr:l}),repeatHeaderHtml:`
    ...
    `}}async function m(e,n,i,a=``){try{let o=await t.getAppSettings(),c=document.getElementById(`print-receipt-container`);c||(c=document.createElement(`div`),c.id=`print-receipt-container`,document.body.appendChild(c));let d=document.getElementById(`stmt-start-date`)?.value||``,m=document.getElementById(`stmt-end-date`)?.value||``,h=d||m?`${d?s(d):`শুরু`} হতে ${m?s(m):`আজ`}`:`সকল লেনদেন`,g=document.getElementById(`stmt-total-bill`)?.innerText||`৳ 0`,_=document.getElementById(`stmt-total-paid`)?.innerText||`৳ 0`,v=document.getElementById(`stmt-total-due`)?.innerText||`৳ 0`,{rowsArray:y,running:b}=f(n,i),{page1HeaderHtml:x,repeatHeaderHtml:S,page1ExtraHtml:C,tableColHeaderHtml:w,signatureHtml:T}=p(e,g,_,v,b,o,`CUSTOMER KHATIYAN`,`কাস্টমার বকেয়া খতিয়ান`,h),E=a&&a.trim()?`
            <div style="margin-top: 15px; padding: 10px 14px; background: #fffbe6; border: 1px solid #ffe58f; border-radius: 8px; font-size: 11px; color: #856404; font-family: sans-serif; page-break-inside: avoid; break-inside: avoid;">
                ...
                ${a.replace(/\n/g,`<br>`)}
            </div>`:``,[D,O,k]=r().split(`-`),A=await l({rowsArray:y,page1HeaderHtml:x,repeatHeaderHtml:S,tableColHeaderHtml:w,page1ExtraHtml:C,summaryHtml:E,signatureHtml:T,formattedDate:`${k}/${O}/${D}`});u(A)}catch(e){console.error(`Statement print error:`,e),d.default.fire(`Error`,`প্রিন্ট করতে সমস্যা হয়েছে`,`error`)}}var h=null;async function g(e){if(!e)return;let t=document.getElementById(`login-screen`),r=document.getElementById(`app-container`);t&&(t.style.display=`none`),r&&r.classList.add(`hidden`);let a=document.getElementById(`public-stmt-view`);a||(a=document.createElement(`div`),a.id=`public-stmt-view`,a.className=`fixed inset-0 z-[9999] overflow-y-auto bg-slate-950 p-3 sm:p-6 font-bn flex flex-col items-center justify-start`,document.body.appendChild(a)),a.innerHTML=`<div class="text-center py-20 text-white font-bold"><i class="fa-solid fa-spinner fa-spin text-2xl text-blue-500 mb-3"></i><p>মেসার্স মা মোটরস্ বিবরণী লোড হচ্ছে...</p></div>`;try{let{CustomerDAO:t,TransactionDAO:r,SettingsDAO:s}=await n(async()=>{let{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}=await import(`./dao-BJg8z6Lo.js`).then(e=>e.u);return{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}},__vite__mapDeps([0,1,2,3,4])),c=await t.getById(e);if(!c)return a.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">কাস্টমার হিসাব পাওয়া যায়নি!</div>`;let l=await s.getAppSettings(),u=(await r.getByCustomer(e)).filter(e=>e.voucherNo!==`OPENING`);u.sort((e,t)=>{let n=new Date(e.date)-new Date(t.date);return n===0?(e.createdAt?.toMillis()||0)-(t.createdAt?.toMillis()||0):n});let d=Number(c.initialDue||0),m=0,g=0,_=0;u.forEach(e=>{e.receivedType===`Less`?_=o(_+(Number(e.paid)||0)):g=o(g+(Number(e.paid)||0)),m=o(m+(Number(e.bill)||0))});let v=o(d+m-g-_),{rowsArray:y}=f(d,u);h={customer:c,docs:u,settings:l,initialDue:d,billSum:m,paidSum:g,lessSum:_,running:v};let{page1HeaderHtml:b,page1ExtraHtml:x}=p(c,`৳ ${i(m)}`,`৳ ${i(g)}`,`৳ ${i(Math.abs(v))} ${v<0?`(Adv)`:``}`,v,l,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`);a.innerHTML=`

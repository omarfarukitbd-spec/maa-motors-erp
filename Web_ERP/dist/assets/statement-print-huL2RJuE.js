...
import{i as e}from"./rolldown-runtime-Dd_uD5pT.js";import{r as t}from"./vendor-COdl40C-.js";import{s as n}from"./dao-DWBGBcb5.js";import{n as r}from"./vendor-ui-n4g2UPZQ.js";import{a as i,i as a,l as o,n as s,o as c,s as l,u}from"./index-B77B9j9v.js";var d=e(r());function f(e,t){let n=[],r=e,i=`<tr style="background:#f1f5f9; border-bottom:2px solid #cbd5e1;">
        ...
        </tr>`;n.push({html:d,textLength:(e.receivedFrom||``).length})}),{rowsArray:n,running:r}}function p(e,t,n,r,a,o,s,c,l){let u=(e.name||``).replace(/^\[.*?\]\s*/,``).trim();return{page1HeaderHtml:i(o,{title:s,subtitle:c,dateRangeStr:l}),repeatHeaderHtml:`
    ...
    `}}async function m(e,t,r,i=``){try{let o=await n.getAppSettings(),u=document.getElementById(`print-receipt-container`);u||(u=document.createElement(`div`),u.id=`print-receipt-container`,document.body.appendChild(u));let d=document.getElementById(`stmt-start-date`)?.value||``,m=document.getElementById(`stmt-end-date`)?.value||``,h=d||m?`${d?c(d):`শুরু`} হতে ${m?c(m):`আজ`}`:`সকল লেনদেন`,g=document.getElementById(`stmt-total-bill`)?.innerText||`৳ 0`,_=document.getElementById(`stmt-total-paid`)?.innerText||`৳ 0`,v=document.getElementById(`stmt-total-due`)?.innerText||`৳ 0`,{rowsArray:y,running:b}=f(t,r),{page1HeaderHtml:x,repeatHeaderHtml:S,page1ExtraHtml:C,tableColHeaderHtml:w,signatureHtml:T}=p(e,g,_,v,b,o,`CUSTOMER KHATIYAN`,`কাস্টমার বকেয়া খতিয়ান`,h),E=i&&i.trim()?`
            <div style="margin-top: 15px; padding: 10px 14px; background: #fffbe6; border: 1px solid #ffe58f; border-radius: 8px; font-size: 11px; color: #856404; font-family: sans-serif; page-break-inside: avoid; break-inside: avoid;">
                ...
                ${i.replace(/\n/g,`<br>`)}
            </div>`:``,[D,O,k]=l().split(`-`),A=await a({rowsArray:y,page1HeaderHtml:x,repeatHeaderHtml:S,tableColHeaderHtml:w,page1ExtraHtml:C,summaryHtml:E,signatureHtml:T,formattedDate:`${k}/${O}/${D}`});s(A)}catch(e){console.error(`Statement print error:`,e),d.default.fire(`Error`,`প্রিন্ট করতে সমস্যা হয়েছে`,`error`)}}var h=null;async function g(e){if(!e)return;let n=document.getElementById(`login-screen`),r=document.getElementById(`app-container`);n&&(n.style.display=`none`),r&&r.classList.add(`hidden`);let i=document.getElementById(`public-stmt-view`);i||(i=document.createElement(`div`),i.id=`public-stmt-view`,i.className=`fixed inset-0 z-[9999] overflow-y-auto bg-slate-950 p-3 sm:p-6 font-bn flex flex-col items-center justify-start`,document.body.appendChild(i)),i.innerHTML=`<div class="text-center py-20 text-white font-bold"><i class="fa-solid fa-spinner fa-spin text-2xl text-blue-500 mb-3"></i><p>মেসার্স মা মোটরস্ বিবরণী লোড হচ্ছে...</p></div>`;try{let{CustomerDAO:n,TransactionDAO:r,SettingsDAO:a}=await t(async()=>{let{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}=await import(`./dao-DWBGBcb5.js`).then(e=>e.d);return{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}},__vite__mapDeps([0,1,2,3,4])),s=await n.getById(e);if(!s)return i.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">কাস্টমার হিসাব পাওয়া যায়নি!</div>`;let c=await a.getAppSettings(),l=(await r.getByCustomer(e)).filter(e=>{let t=String(e.voucherNo||``).trim().toUpperCase();return t!==`OPENING`&&t!==`OPEN`&&t!==`প্রারম্ভিক ব্যালেন্স`&&t!==`প্রারম্ভিক জের`});l.sort((e,t)=>{let n=new Date(e.date)-new Date(t.date);return n===0?(e.createdAt?.toMillis()||0)-(t.createdAt?.toMillis()||0):n});let d=Number(s.initialDue||0),m=0,g=0,_=0;l.forEach(e=>{e.receivedType===`Less`?_=u(_+(Number(e.paid)||0)):g=u(g+(Number(e.paid)||0)),m=u(m+(Number(e.bill)||0))});let v=u(d+m-g-_),{rowsArray:y}=f(d,l);h={customer:s,docs:l,settings:c,initialDue:d,billSum:m,paidSum:g,lessSum:_,running:v};let{page1HeaderHtml:b,page1ExtraHtml:x}=p(s,`৳ ${o(m)}`,`৳ ${o(g)}`,`৳ ${o(Math.abs(v))} ${v<0?`(Adv)`:``}`,v,c,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`);i.innerHTML=`

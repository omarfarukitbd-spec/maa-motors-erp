...
_Maa Motors ERP সিস্টেম থেকে স্বয়ংক্রিয়ভাবে প্রস্তুতকৃত_`,u=(t.ownerPhone||t.shopPhone||``).split(/[,/]/).map(e=>e.trim()).filter(Boolean),d=u[0]||``,f=u[1]||``,p=`
    ...
    `;try{let r=await as(e);if(!r||r.length===0){n&&n.classList.add(`hidden`),t.innerHTML=`
            ...
            `,window._currentMemoSearchResults=r):n&&n.classList.add(`hidden`),t.innerHTML=ds(i,a)}catch(e){console.error(`executeMemoSearch error:`,e),t.innerHTML=`<div class="p-8 text-center text-red-400 font-bold bg-slate-900/60 rounded-3xl border border-red-500/20">মেমো অনুসন্ধানে সমস্যা হয়েছে</div>`}}}window.triggerMemoVoiceSearch=function(){fs((e,t)=>{let n=document.getElementById(`memo-search-input`);n&&(n.value=e,document.getElementById(`memo-search-clear-btn`)?.classList.remove(`hidden`),_s(e)),M(`ভয়েস রিসিভ: "${t}"`,`success`)})};async function vs(){let e=document.getElementById(`memo-recent-chips`);if(e)try{let t=await cs(8);if(t.length===0){e.innerHTML=`<span class="text-slate-600 text-xs">কোনো মেমো নেই</span>`;return}e.innerHTML=t.map(e=>`
            <button onclick="window.searchMemoDirectly('${j(e.voucherNo)}')" class="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-cyan-600/30 hover:border-cyan-500/50 text-slate-300 hover:text-cyan-300 border border-slate-700/60 text-[11px] font-mono font-bold transition-all shrink-0 cursor-pointer flex items-center gap-1.5" title="${j(e.customerName)} (${A(e.date)})">
                <span>#${j(e.voucherNo)}</span>
                <span class="text-[10px] text-slate-500 font-sans font-normal">${j(e.customerName.slice(0,10))}</span>
            </button>

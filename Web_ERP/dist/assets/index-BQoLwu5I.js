                ...
                `})}n.innerHTML=s,r&&(r.innerText=`৳ ${I(o)}`);let l=document.getElementById(`dash-collection-card-total`),u=document.getElementById(`dash-collection-card-words`);l&&(l.innerText=`৳ ${I(o)}`),u&&(u.innerText=o>0?`${_(o)}`:`শূন্য টাকা মাত্র`);let d=document.getElementById(`dash-collection-method-cards`);if(d){let e=``;if(a.length>0&&Object.keys(c).length>0){e+=`
                    <div class="method-card-btn bg-emerald-600/90 backdrop-blur-md border border-emerald-500/50 rounded-2xl p-3 sm:p-3.5 cursor-pointer hover:bg-emerald-500 transition-all flex flex-col justify-between gap-1.5 w-full min-w-0 shadow-[0_5px_15px_-5px_rgba(16,185,129,0.5)] ring-2 ring-emerald-400" onclick="window.filterCollectionByMethod('All')" data-method="All">
                        ...
                        <div class="flex items-end justify-between gap-2">
                            <span class="text-sm sm:text-base font-black text-white tracking-tight truncate font-inter">৳ ${I(o)}</span>
                            <span class="text-[9px] bg-emerald-900/60 text-emerald-100 px-1.5 py-0.5 rounded-md font-bold shrink-0">${a.filter(e=>e.receivedType!==`Less`).length} জন</span>
                        </div>
                    </div>

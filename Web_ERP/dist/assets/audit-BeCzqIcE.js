const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/dao-t3V9h23g.js","assets/rolldown-runtime-Dd_uD5pT.js","assets/vendor-firebase-F_CbrIUB.js","assets/vendor-CmrCKYVr.js","assets/vendor-CwbMEznW.css"])))=>i.map(i=>d[i]);
import{n as e}from"./rolldown-runtime-Dd_uD5pT.js";import{t}from"./vendor-firebase-F_CbrIUB.js";import{a as n}from"./vendor-CmrCKYVr.js";import{l as r,s as i,t as a}from"./dao-t3V9h23g.js";async function o(){try{return(await navigator.mediaDevices.getUserMedia({video:!0})).getTracks().forEach(e=>e.stop()),!0}catch(e){return console.warn(`Camera permission denied or camera not found:`,e),!1}}async function s(){return new Promise(async e=>{try{let t=await navigator.mediaDevices.getUserMedia({video:{facingMode:`user`}}),n=document.createElement(`video`);n.srcObject=t,n.autoplay=!0,n.muted=!0,n.playsInline=!0,n.style.display=`none`,document.body.appendChild(n),n.onloadedmetadata=()=>{n.play(),setTimeout(()=>{let r=document.createElement(`canvas`);r.width=n.videoWidth,r.height=n.videoHeight,r.getContext(`2d`).drawImage(n,0,0,r.width,r.height),t.getTracks().forEach(e=>e.stop()),n.remove(),r.toBlob(t=>{e(t)},`image/jpeg`,.8)},500)}}catch(t){console.error(`Failed to capture photo:`,t),e(null)}})}async function c(e,r,i,o,c={},l=null){try{let u=t.auth().currentUser,d=typeof navigator<`u`&&navigator.userAgent&&/Mobi|Android|iPhone/i.test(navigator.userAgent),f={action:e,module:r,entityId:i||``,entityName:o||``,details:c||{},deviceInfo:d?`Mobile`:`Desktop`,userEmail:u?u.email:`Unknown`,userId:u?u.uid:`System`,timestamp:t.firestore.FieldValue.serverTimestamp(),clientTimestamp:new Date().toISOString()};if(l&&(f.changes=l),await a.add(f),console.log(`[Audit] ${e} on ${r} (${o}) logged successfully.`),[`DELETE`,`UPDATE`,`SECURITY_ALERT`,`LOGIN`].includes(e))try{let t=await(await n(()=>import(`./dao-t3V9h23g.js`).then(e=>e.d),__vite__mapDeps([0,1,2,3,4]))).SettingsDAO.getAppSettings(),a=t.telegramBotToken,l=t.telegramChatId;if(a&&l){let t=`[INFO]`;e===`DELETE`||e===`SECURITY_ALERT`?t=`[ALERT]`:e===`UPDATE`?t=`[WARN]`:e===`LOGIN`&&(t=`[SUCCESS]`);let n=``;c&&typeof c==`object`&&Object.keys(c).length>0&&(n=`\n*Details:* \`${JSON.stringify(c).substring(0,100)}\``);let u=`
${t} *Maa Motors ERP Alert*
*Action:* ${e}
*Module:* ${r||`Unknown`}
*Target:* ${o||i||`Unknown`}
*User:* ${f.userEmail}
*Time:* ${new Date().toLocaleString(`en-US`,{timeZone:`Asia/Dhaka`})}${n}
                    `.trim(),d=null;if(e===`SECURITY_ALERT`&&(d=await s()),d){let e=new FormData;e.append(`chat_id`,l),e.append(`photo`,d,`intruder.jpg`),e.append(`caption`,u),e.append(`parse_mode`,`Markdown`),await fetch(`https://api.telegram.org/bot${a}/sendPhoto`,{method:`POST`,body:e})}else await fetch(`https://api.telegram.org/bot${a}/sendMessage`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({chat_id:l,text:u,parse_mode:`Markdown`})})}}catch(e){console.error(`Failed to send Telegram alert:`,e)}}catch(e){console.error(`Failed to write audit log:`,e)}}function l(e){return e.deviceInfo===`Mobile`||e.details&&e.details.device===`Mobile`?`<span class="inline-flex items-center gap-1 text-[10px] text-amber-400 bg-amber-400/10 border border-amber-400/20 px-1.5 py-0.5 rounded font-mono" title="Mobile Device"><i class="fa-solid fa-mobile-screen"></i> Mobile</span>`:`<span class="inline-flex items-center gap-1 text-[10px] text-sky-400 bg-sky-400/10 border border-sky-400/20 px-1.5 py-0.5 rounded font-mono" title="Desktop / PC"><i class="fa-solid fa-desktop"></i> Desktop</span>`}function u(e){if(!e)return`<span class="text-slate-500">-</span>`;let t=`<div class="font-bn space-y-1">`;if(e.entityName&&(t+=`<div class="flex items-center gap-2 justify-between"><div class="text-xs font-black text-white tracking-tight">${e.entityName}</div>${l(e)}</div>`),e.changes){let{old:n={},new:r={}}=e.changes,i=Array.from(new Set([...Object.keys(n),...Object.keys(r)]));i.length>0&&(t+=`<div class="mt-1 space-y-1 text-[11px] font-sans">`,i.forEach(e=>{let i=n[e]===void 0?`N/A`:String(n[e]),a=r[e]===void 0?`N/A`:String(r[e]);i!==a&&(t+=`
    ...
    `}async function g(e,t=`সকল অডিট রেকর্ড`){if(!e||e.length===0){window.Swal&&window.Swal.fire(`ফাঁকা রিপোর্ট`,`প্রিন্ট করার জন্য কোনো অডিট রেকর্ড পাওয়া যায়নি`,`warning`);return}try{let n=await i.getAppSettings(),r=n.shopName||`M/S. Maa Motors`,a=n.shopAddress||`Shop No. 22, Rahman Tower, 1st Rail Gate, Muradpur, Hathazari Road`,o=n.shopPhone||`01819-397669, 01815-707934`,s=new Date().toLocaleDateString(`en-GB`),c=e.map((e,t)=>{let n=e.timestamp?e.timestamp.toDate():new Date(e.clientTimestamp||Date.now()),r=n.toLocaleDateString(`en-GB`)+` `+n.toLocaleTimeString([],{hour:`2-digit`,minute:`2-digit`}),i=e.entityName||`-`;return e.changes?i+=` (Old: ${JSON.stringify(e.changes.old)} => New: ${JSON.stringify(e.changes.new)})`:e.details&&Object.keys(e.details).length>0&&(i+=` (${JSON.stringify(e.details)})`),`
        ...
        `,u=window.open(``,`_blank`);u&&(u.document.write(l),u.document.close(),u.focus(),setTimeout(()=>{u.print()},500))}catch(e){console.error(`Print audit report error:`,e),window.Swal&&window.Swal.fire(`এরর`,`অডিট রিপোর্ট প্রিন্ট করতে সমস্যা হয়েছে`,`error`)}}var _=e({applyAuditFilters:()=>M,auditLog:()=>c,getRecentAuditLogs:()=>O,renderAuditLogs:()=>A,unsubscribeAuditLogs:()=>D}),v=[],y=[],b=[],x=`all`,S=null,C=[],w=1,T=30,E=!1;function D(){v.forEach(e=>{typeof e==`function`&&e()}),v=[]}async function O(e=50){try{return await a.getRecent(e)}catch(e){return console.error(`Failed to fetch audit logs:`,e),[]}}async function k(e){let t=document.getElementById(`audit-user-select`);if(!t)return;let n=[];try{n=await r.getAll()}catch(e){console.warn(`Could not fetch user list for audit dropdown:`,e)}let i=n.map(e=>e.email).filter(Boolean),a=e.map(e=>e.userEmail).filter(Boolean);t.innerHTML=`<option value="">-- সকল ইউজার (All Staff) --</option>`+Array.from(new Set([...i,...a])).sort().map(e=>`<option value="${e}">${e}</option>`).join(``)}async function A(e){if(window.AppState.currentUserRole===`Staff`&&window.AppState.permissions.viewAuditLog===!1){e.innerHTML=`<div class="m3-card text-center font-bn py-12"><h2 class="text-xl font-black text-red-500">অ্যাক্সেস ডিনাইড!</h2></div>`;return}D(),e.innerHTML=`
    ...
    `,j()}async function j(e=`next`){let t=document.getElementById(`audit-logs-list`);if(t)if(E)try{y.length<100&&(y=await a.getRecent(300)),M()}catch(e){console.error(e)}else{t.innerHTML=`<tr><td colspan="5" class="text-center py-16 text-slate-400 italic"><i class="fa-solid fa-spinner fa-spin text-2xl text-blue-500 mb-2 block"></i>অডিট ডাটা লোড হচ্ছে...</td></tr>`;try{let n=e===`next`?S:C.length>1?C[C.length-2]:null,r=await a.getByPage(T,n,`timestamp`,`desc`);S=r.lastDoc,e===`next`?n&&C.push(n):C.pop();let i=document.getElementById(`audit-pagination`);i&&(i.classList.remove(`hidden`),document.getElementById(`audit-current-page-display`).innerText=w,document.getElementById(`audit-prev-page`).disabled=w===1,document.getElementById(`audit-next-page`).disabled=r.count<T),y=r.data,N(y,t);let o=p(y),s=document.getElementById(`audit-stats-cards-container`);s&&(s.innerHTML=m(o));let c=document.getElementById(`audit-tabs-container`);c&&!c.innerHTML.trim()&&(c.innerHTML=h(x)),await k(y),b=y}catch(e){console.error(`Load audit error:`,e),t.innerHTML=`<tr><td colspan="5" class="text-center py-12 text-red-400 font-bold">ডাটা লোড করতে সমস্যা হয়েছে</td></tr>`}}}async function M(){let e=document.getElementById(`audit-logs-list`);if(!e)return;let t=document.getElementById(`audit-search-input`)?.value||``,n=document.getElementById(`audit-user-select`)?.value||``,r=document.getElementById(`audit-action-select`)?.value||``,i=document.getElementById(`audit-module-select`)?.value||``,o=document.getElementById(`audit-start-date`)?.value||``,s=document.getElementById(`audit-end-date`)?.value||``;x===`critical`&&(r||=`DELETE`);let c=t||n||r||i||o||s||x!==`all`,l=document.getElementById(`audit-pagination`);if(!c){E=!1,l&&l.classList.remove(`hidden`),S=null,C=[],w=1,j();return}E=!0,l&&l.classList.add(`hidden`),y.length<100&&(e.innerHTML=`<tr><td colspan="5" class="text-center py-16 text-slate-400 italic"><i class="fa-solid fa-spinner fa-spin text-2xl text-blue-500 mb-2 block"></i>খুঁজছি...</td></tr>`,y=await a.getRecent(300)),b=d(y,{searchQuery:t,userFilter:n,actionFilter:r,moduleFilter:i,startDate:o,endDate:s}),N(b,e)}function N(e,t){if(e.length===0){t.innerHTML=`<tr><td colspan="5" class="text-center py-16 text-slate-500 font-bold italic">কোনো ফিল্টারকৃত অডিট রেকর্ড পাওয়া যায়নি।</td></tr>`;return}t.innerHTML=e.map(e=>{let t=e.timestamp?e.timestamp.toDate():new Date(e.clientTimestamp||Date.now()),n=t.toLocaleDateString(`en-GB`),r=t.toLocaleTimeString([],{hour:`2-digit`,minute:`2-digit`,second:`2-digit`}),i=``;return i=e.action===`CREATE`?`<span class="bg-emerald-500/20 text-emerald-400 px-2.5 py-1 rounded-xl text-[10px] font-black border border-emerald-500/30">CREATE</span>`:e.action===`UPDATE`?`<span class="bg-amber-500/20 text-amber-400 px-2.5 py-1 rounded-xl text-[10px] font-black border border-amber-500/30">UPDATE</span>`:e.action===`DELETE`?`<span class="bg-red-500/20 text-red-400 px-2.5 py-1 rounded-xl text-[10px] font-black border border-red-500/30">DELETE</span>`:e.action===`LOGIN`?`<span class="bg-blue-500/20 text-blue-400 px-2.5 py-1 rounded-xl text-[10px] font-black border border-blue-500/30">LOGIN</span>`:`<span class="bg-slate-800 text-slate-300 px-2.5 py-1 rounded-xl text-[10px] font-black border border-slate-700">${e.action}</span>`,`
            <tr class="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                ...
                <td class="p-3.5 min-w-[280px]">${u(e)}</td>
            </tr>
        `}).join(``)}window.switchAuditTab=e=>{x=e;let t=document.getElementById(`audit-tabs-container`);t&&(t.innerHTML=h(x)),M()},window.changeAuditPage=e=>{e===`next`?w++:w--,j(e)},window.auditLog=c,window.getRecentAuditLogs=O,window.unsubscribeAuditLogs=D,window.applyAuditFilters=M,window.refreshAuditLogsList=()=>{S=null,C=[],w=1,y=[],j()},window.exportActiveAuditExcel=()=>f(b),window.triggerPrintAuditLogReport=()=>g(b);export{o as a,c as i,A as n,D as r,_ as t};
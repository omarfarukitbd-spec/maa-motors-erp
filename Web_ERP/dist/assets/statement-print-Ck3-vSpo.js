...
import{i as e}from"./rolldown-runtime-Dd_uD5pT.js";import{i as t}from"./dao-CJcWJLH8.js";import{c as n,d as r,o as i,s as a}from"./ui-helpers-BkH1xB3j.js";import"./utils-Nckx6ucv.js";import{n as o}from"./vendor-ui-n4g2UPZQ.js";import{r as s,t as c}from"./smart-print-engine-B0360BA8.js";import{t as l}from"./index-BGGyewiP.js";var u=e(o());function d(e,t){let n=[],i=e,o=`<tr style="background:#f1f5f9; border-bottom:2px solid #cbd5e1;">
        ...
        <td style="text-align:right; font-weight:900; color:#0f172a; padding: 8px 12px; border-left:1px solid #cbd5e1; background:#fff;">
    ...
    </tr>`;return n.push({html:o,textLength:15}),t.forEach(e=>{let t=Number(e.bill)||0,o=Number(e.paid)||0,s=e.receivedType||``;i+=t-o;let c=o>0?s===`Less`?`<strong style="color:#0f172a;">[LESS]</strong>`:`<strong style="color:#0f172a;">${s}</strong>`:`-`;o>0&&e.receivedFrom&&(c+=`<br><span style="font-size:9px;color:#475569;">${e.receivedFrom}</span>`);let l=e.voucherNo&&e.voucherNo!==`OPENING`?`<br><span style="font-size:9px;color:#0f172a;font-weight:900;">#${e.voucherNo}</span>`:``,u=`<tr>
            <td style="font-size:11px; border-bottom:1px solid #e2e8f0; padding: 6px 12px; color:#0f172a;">${a(e.date)}</td>
            <td style="font-size:11px; border-bottom:1px solid #e2e8f0; padding: 6px 12px; color:#0f172a;">${c}${l}</td>
            <td style="text-align:right; color:#dc2626; font-weight:700; border-bottom:1px solid #e2e8f0; padding: 6px 12px;">${t>0?r(t):`-`}</td>
            <td style="text-align:right; color:#059669; font-weight:700; border-bottom:1px solid #e2e8f0; padding: 6px 12px;">${o>0?r(o):`-`}</td>
            <td style="text-align:right; font-weight:900; color:#0f172a; border-bottom:1px solid #e2e8f0; padding: 6px 12px; border-left:1px solid #e2e8f0;">
                ৳ ${r(Math.abs(i))} ${i<0?`<span style="font-size:8px;">(Adv)</span>`:``}
            </td>
        </tr>`;n.push({html:u,textLength:(e.receivedFrom||``).length})}),{rowsArray:n,running:i}}function f(e,t,n,r,a,o,s,c,l){return{page1HeaderHtml:i(o,{title:s,subtitle:c,dateRangeStr:l}),repeatHeaderHtml:`
    ...
    `}}async function p(e,r,i,o=``){try{let l=await t.getAppSettings(),u=document.getElementById(`print-receipt-container`);u||(u=document.createElement(`div`),u.id=`print-receipt-container`,document.body.appendChild(u));let p=document.getElementById(`stmt-start-date`)?.value||``,m=document.getElementById(`stmt-end-date`)?.value||``,h=p||m?`${p?a(p):`শুরু`} হতে ${m?a(m):`আজ`}`:`সকল লেনদেন`,g=document.getElementById(`stmt-total-bill`)?.innerText||`৳ 0`,_=document.getElementById(`stmt-total-paid`)?.innerText||`৳ 0`,v=document.getElementById(`stmt-total-due`)?.innerText||`৳ 0`,{rowsArray:y,running:b}=d(r,i),{page1HeaderHtml:x,repeatHeaderHtml:S,page1ExtraHtml:C,tableColHeaderHtml:w,signatureHtml:T}=f(e,g,_,v,b,l,`CUSTOMER KHATIYAN`,`কাস্টমার বকেয়া খতিয়ান`,h),E=o&&o.trim()?`
            <div style="margin-top: 15px; padding: 10px 14px; background: #fffbe6; border: 1px solid #ffe58f; border-radius: 8px; font-size: 11px; color: #856404; font-family: sans-serif; page-break-inside: avoid; break-inside: avoid;">
                ...
                ${o.replace(/\n/g,`<br>`)}
            </div>`:``,[D,O,k]=n().split(`-`),A=await s({rowsArray:y,page1HeaderHtml:x,repeatHeaderHtml:S,tableColHeaderHtml:w,page1ExtraHtml:C,summaryHtml:E,signatureHtml:T,formattedDate:`${k}/${O}/${D}`});c(A)}catch(e){console.error(`Statement print error:`,e),u.default.fire(`Error`,`প্রিন্ট করতে সমস্যা হয়েছে`,`error`)}}var m=null;async function h(e){if(!e)return;let t=document.getElementById(`login-screen`),n=document.getElementById(`app-container`);t&&(t.style.display=`none`),n&&n.classList.add(`hidden`);let i=document.getElementById(`public-stmt-view`);i||(i=document.createElement(`div`),i.id=`public-stmt-view`,i.className=`fixed inset-0 z-[9999] overflow-y-auto bg-slate-950 p-3 sm:p-6 font-bn flex flex-col items-center justify-start`,document.body.appendChild(i)),i.innerHTML=`<div class="text-center py-20 text-white font-bold"><i class="fa-solid fa-spinner fa-spin text-2xl text-blue-500 mb-3"></i><p>মেসার্স মা মোটরস্ বিবরণী লোড হচ্ছে...</p></div>`;try{let{CustomerDAO:t,TransactionDAO:n,SettingsDAO:a}=await l(async()=>{let{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}=await import(`./dao-CJcWJLH8.js`).then(e=>e.c);return{CustomerDAO:e,TransactionDAO:t,SettingsDAO:n}},__vite__mapDeps([0,1,2,3,4])),o=await t.getById(e);if(!o)return i.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">কাস্টমার হিসাব পাওয়া যায়নি!</div>`;let s=await a.getAppSettings(),c=(await n.getByCustomer(e)).filter(e=>e.voucherNo!==`OPENING`);c.sort((e,t)=>{let n=new Date(e.date)-new Date(t.date);return n===0?(e.createdAt?.toMillis()||0)-(t.createdAt?.toMillis()||0):n});let u=Number(o.initialDue||0),p=0,h=0,g=0;c.forEach(e=>{e.receivedType===`Less`?g+=Number(e.paid)||0:h+=Number(e.paid)||0,p+=Number(e.bill)||0});let _=u+p-h-g,{rowsArray:v}=d(u,c);m={customer:o,docs:c,settings:s,initialDue:u,billSum:p,paidSum:h,lessSum:g,running:_};let{page1HeaderHtml:y,page1ExtraHtml:b}=f(o,`৳ ${r(p)}`,`৳ ${r(h)}`,`৳ ${r(Math.abs(_))} ${_<0?`(Adv)`:``}`,_,s,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`);i.innerHTML=`
        ...
        `}catch(e){console.error(e),i.innerHTML=`<div class="m3-card text-center py-12 text-red-400 font-bold max-w-md mx-auto">স্টেটমেন্ট লোড করতে ব্যর্থ!</div>`}}window.printPublicStatement=async()=>{if(!m)return;let{customer:e,docs:t,settings:i,initialDue:a,billSum:o,paidSum:l,running:u}=m,p=document.getElementById(`print-receipt-container`);p||(p=document.createElement(`div`),p.id=`print-receipt-container`,document.body.appendChild(p));let{rowsArray:h}=d(a,t),{page1HeaderHtml:g,repeatHeaderHtml:_,page1ExtraHtml:v,tableColHeaderHtml:y,signatureHtml:b}=f(e,`৳ ${r(o)}`,`৳ ${r(l)}`,`৳ ${r(Math.abs(u))} ${u<0?`(Adv)`:``}`,u,i,`STATEMENT SUMMARY`,`সকল লেনদেন`,`সকল লেনদেন`),[x,S,C]=n().split(`-`),w=await s({rowsArray:h,page1HeaderHtml:g,repeatHeaderHtml:_,tableColHeaderHtml:y,page1ExtraHtml:v,summaryHtml:``,signatureHtml:b,formattedDate:`${C}/${S}/${x}`});c(w)},window.renderPublicStatementView=h;export{p as printStatement,h as renderPublicStatementView};
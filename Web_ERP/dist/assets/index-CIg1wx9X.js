...
<style>${Ee}${t}</style>
</head><body>${e}</body></html>`),a.close();let o=()=>{try{i.contentWindow.focus(),i.contentWindow.print()}catch{window.print()}setTimeout(()=>{try{i.remove()}catch(e){console.error(`Remove iframe error:`,e)}},4e3)},s=i.contentDocument;s&&s.fonts&&s.fonts.ready?(async()=>{try{await s.fonts.ready,o()}catch(e){console.error(`Fonts ready error:`,e),setTimeout(o,700)}})():setTimeout(o,700)}function H(e,t=`একটি সমস্যা হয়েছে`){console.error(`[App Error]`,e);let n=e.message||String(e);e.code===`permission-denied`?n=`আপনার এই কাজটি করার অনুমতি নেই। (Permission Denied)`:e.code===`unavailable`?n=`ইন্টারনেট কানেকশন চেক করুন। (Service Unavailable)`:e.code===`resource-exhausted`?n=`কোটা শেষ হয়ে গেছে। (Quota Exceeded)`:e.message&&e.message.includes(`index`)&&(n=`ডাটাবেস ইনডেক্স প্রয়োজন। অনুগ্রহ করে অ্যাডমিনের সাথে যোগাযোগ করুন।`),z.default.fire({title:`ভুল হয়েছে!`,html:`

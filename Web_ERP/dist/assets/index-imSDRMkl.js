...
_Maa Motors ERP সিস্টেম থেকে স্বয়ংক্রিয়ভাবে প্রস্তুতকৃত_`,u=(t.ownerPhone||t.shopPhone||``).split(/[,/]/).map(e=>e.trim()).filter(Boolean),d=u[0]||``,f=u[1]||``,p=`
    ...
    `;let a=document.getElementById(`fs-date-range-input`);a&&typeof flatpickr<`u`&&(Qo=flatpickr(a,{mode:`range`,dateFormat:`Y-m-d`,altInput:!0,altFormat:`d/m/Y`,defaultDate:[r,i],onClose:e=>{if(e.length===2){let t=D(e[0]),n=D(e[1]);M(`সময়কাল: ${A(t)} থেকে ${A(n)}`,`info`,`ফিল্টার`),window.fsLoadData(t,n)}else if(e.length===1){let t=D(e[0]);M(`তারিখ: ${A(t)}`,`info`,`ফিল্টার`),window.fsLoadData(t,t)}}})),window.fsLoadData=async(e,t,n=0)=>{let r=document.getElementById(`fs-selected-period-text`);r&&(r.innerHTML=`<i class="fa-solid fa-spinner fa-spin text-emerald-400 mr-1"></i> হিসাব লোড হচ্ছে...`);try{X=await Bo(e,t),es(X);try{ts()}catch(e){console.warn(`Aging tab render warning:`,e)}}catch(i){console.error(`FS load attempt error:`,i),n<2?setTimeout(()=>{window.fsLoadData(e,t,n+1)},600):(M(`ডাটা লোড করতে সমস্যা হয়েছে! ইন্টারনেট সংযোগ চেক করুন।`,`error`,`আর্থিক রিপোর্ট`),r&&(r.innerText=`ডাটা লোড করতে সমস্যা হয়েছে।`))}},window.fsSetPeriod=(e,t=!1)=>{document.querySelectorAll(`.fs-preset-btn`).forEach(t=>{t.className=t.getAttribute(`data-period`)===e?`fs-preset-btn px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-black bg-emerald-500 text-slate-950 shadow-md transition-all flex-1 sm:flex-none text-center cursor-pointer`:`fs-preset-btn px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold text-slate-300 hover:text-white hover:bg-slate-800 transition-all active:scale-95 flex-1 sm:flex-none text-center cursor-pointer`});let n=new Date,r=E(),i=E(),a=`আজকের`;if(e===`today`)r=i=E(),a=`আজকের`;else if(e===`yesterday`){let e=new Date;e.setDate(e.getDate()-1),r=i=D(e),a=`গতকালের`}else if(e===`this_week`){let e=new Date;e.setDate(e.getDate()-6),r=D(e),i=E(),a=`এই সপ্তাহের`}else e===`this_month`?(r=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}-01`,i=E(),a=`চলতি মাসের`):e===`this_year`&&(r=`${n.getFullYear()}-01-01`,i=E(),a=`চলতি বছরের`);Qo&&Qo.setDate([r,i],!1),t&&M(`${a} হিসাব লোড করা হচ্ছে...`,`info`,`সময়কাল ফিল্টার`),window.fsLoadData(r,i)},window.fsSwitchTab=e=>{document.querySelectorAll(`.fs-tab-btn`).forEach(e=>{e.className=`fs-tab-btn px-3.5 py-2 rounded-xl text-xs font-bold text-slate-400 hover:text-white hover:bg-slate-800 transition-all flex items-center gap-1.5 shrink-0 cursor-pointer`});let t=document.getElementById(`fs-tab-btn-${e}`);t&&(t.className=`fs-tab-btn px-3.5 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 shrink-0 bg-emerald-500 text-slate-950 shadow-md cursor-pointer`),[`customers`,`dayByDay`,`aging`,`expenses`,`methods`].forEach(t=>{let n=document.getElementById(`fs-tab-content-${t}`);n&&n.classList.toggle(`hidden`,t!==e)})},window.fsFilterCustomerRows=e=>{let t=(e||``).toLowerCase().trim(),n=0;document.querySelectorAll(`.fs-cust-row`).forEach(e=>{let r=e.innerText.toLowerCase().includes(t);e.style.display=r?``:`none`,r&&n++});let r=document.getElementById(`fs-cust-table-status`);r&&(r.innerText=t?`ফিল্টার অনুযায়ী পাওয়া গেছে: ${n} জন`:``)},window.fsOpenCashCounter=()=>{if(!X)return M(`হিসাব প্রস্তুত হচ্ছে, এক মুহূর্ত অপেক্ষা করুন...`,`info`,`ক্যাশ কাউন্টার`);Wo(X?X.cashCollection:0)},window.fsShareWhatsApp=()=>{if(!X)return M(`হিসাব প্রস্তুত হচ্ছে, এক মুহূর্ত অপেক্ষা করুন...`,`info`,`WhatsApp ক্লোজিং`);Ko(X)},window.fsPrintCustomerRegister=()=>{if(!X)return M(`ডাটা লোড হচ্ছে, অপেক্ষা করুন...`,`info`,`প্রিন্ট`);if(X.customerCollections.length===0)return M(`নির্বাচিত তারিখে প্রিন্ট করার মতো কোনো কাস্টমার আদায়ের রেকর্ড নেই!`,`warning`,`প্রিন্ট`);M(`কাস্টমার আদায় শিট প্রিন্ট প্রস্তুত হচ্ছে...`,`info`,`প্রিন্ট`),Vo(X)},window.fsPrintMonthlyAudit=()=>{if(!X)return M(`ডাটা লোড হচ্ছে, অপেক্ষা করুন...`,`info`,`প্রিন্ট`);if(X.dayByDaySummary.length===0)return M(`নির্বাচিত সময়ে প্রিন্ট করার মতো কোনো দৈনিক সারাংশ রেকর্ড নেই!`,`warning`,`প্রিন্ট`);M(`অডিট শিট প্রিন্ট প্রস্তুত হচ্ছে...`,`info`,`প্রিন্ট`),Ho(X)},window.fsExportExcel=()=>{if(!X)return M(`ডাটা লোড হচ্ছে, অপেক্ষা করুন...`,`info`,`Excel`);if(X.customerCollections.length===0)return M(`এক্সেল ডাউনলোড করার মতো কোনো আদায়ের রেকর্ড নেই!`,`warning`,`Excel`);if(!window.XLSX)return M(`এক্সেল ইঞ্জিন লোড হচ্ছে, পুনরায় চেষ্টা করুন...`,`warning`,`Excel`);try{let e=X.customerCollections.map((e,t)=>({SL:t+1,তারিখ:e.date,"A/C নং":e.customerAccountNo,"কাস্টমারের নাম":e.customerName,মোবাইল:e.customerPhone,জোন:e.customerZone,"ভাউচার নং":e.voucherNo,"পেমেন্ট মেথড":e.receivedType,"আদায় (৳)":e.amount,"অবশিষ্ট বাকি (৳)":e.currentDue})),t=XLSX.utils.json_to_sheet(e),n=XLSX.utils.book_new();XLSX.utils.book_append_sheet(n,t,`আদায় তালিকা`),XLSX.writeFile(n,`Maa_Motors_Collection_${X.startDate}_to_${X.endDate}.xlsx`),M(`এক্সেল ফাইল ডাউনলোড সম্পন্ন হয়েছে!`,`success`,`Excel`)}catch(e){console.error(`Excel error:`,e),M(`এক্সেল ফাইল তৈরিতে সমস্যা হয়েছে!`,`error`,`Excel`)}},window.fsSetPeriod(`today`,!1)}function es(e){let t=document.getElementById(`fs-selected-period-text`);t&&(t.innerHTML=e.startDate===e.endDate?`<i class="fa-solid fa-calendar-day text-emerald-400 mr-1"></i> তারিখ: <strong class="text-white">${A(e.startDate)}</strong>`:`<i class="fa-solid fa-calendar-week text-emerald-400 mr-1"></i> সময়কাল: <strong class="text-white">${A(e.startDate)}</strong> থেকে <strong class="text-white">${A(e.endDate)}</strong>`),document.getElementById(`fs-kpi-sales`).innerText=`৳ ${k(e.totalSales)}`,document.getElementById(`fs-kpi-sales-sub`).innerText=`${e.salesCount} টি ইনভয়েস`,document.getElementById(`fs-kpi-collection`).innerText=`৳ ${k(e.totalCollection)}`,document.getElementById(`fs-kpi-collection-sub`).innerText=`ক্যাশ: ৳ ${k(e.cashCollection)} | ব্যাংক: ৳ ${k(e.bankCollection)}`,document.getElementById(`fs-kpi-expense`).innerText=`৳ ${k(e.totalExpenses)}`,document.getElementById(`fs-kpi-expense-sub`).innerText=`${e.rawExpenses.length} টি খরচের ভাউচার`;let n=document.getElementById(`fs-kpi-net`);n.innerText=`৳ ${k(e.netCashFlow)}`,n.className=e.netCashFlow>=0?`text-lg sm:text-2xl lg:text-3xl font-black text-emerald-400 tracking-tight font-inter`:`text-lg sm:text-2xl lg:text-3xl font-black text-rose-400 tracking-tight font-inter`;let r=document.getElementById(`fs-badge-cust-count`);r&&(r.innerText=`${e.customerCollections.length} জন`);let i=document.getElementById(`fs-badge-days-count`);i&&(i.innerText=`${e.dayByDaySummary.length} দিন`);let a=document.getElementById(`fs-customer-tbody`);a&&(a.innerHTML=e.customerCollections.length===0?`<tr><td colspan="9" class="text-center py-10 text-slate-500 italic">এই সময়ের মধ্যে কোনো কাস্টমার আদায় নেই।</td></tr>`:e.customerCollections.map((e,t)=>{let n=e.receivedType===`Cash`?`<span class="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded text-[10px] font-bold whitespace-nowrap"><i class="fa-solid fa-hand-holding-dollar mr-1"></i> ক্যাশ</span>`:`<span class="bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded text-[10px] font-bold whitespace-nowrap"><i class="fa-solid fa-building-columns mr-1"></i> ${e.receivedFrom||`Bank`}</span>`;return`
        ...
        `}).join(``)}}function rs(e,t={}){$o(e,t)}function is(e=``){return String(e||``).trim().replace(/^[#№\s]+/,``).replace(/^(INV|MEMO|VOUCHER|BILL)[-_\s]*/i,``).trim()}async function as(e){let t=is(e);if(!t||t.length<1)return[];try{let e=await o.collection.where(`voucherNo`,`==`,t).get(),n=[];return e.forEach(e=>n.push({id:e.id,...e.data()})),n.length===0&&t.toLowerCase()!==t.toUpperCase()&&(await o.collection.where(`voucherNo`,`==`,t.toUpperCase()).get()).forEach(e=>n.push({id:e.id,...e.data()})),n.length===0&&t.length>=2&&(await o.collection.where(`voucherNo`,`>=`,t).where(`voucherNo`,`<=`,t+``).limit(10).get()).forEach(e=>{n.some(t=>t.id===e.id)||n.push({id:e.id,...e.data()})}),await Promise.all(n.map(async e=>await os(e)))}catch(e){return console.error(`searchMemosByNumber error:`,e),[]}}async function os(e){if(!e)return null;let t={};if(e.customerId)try{t=await a.getById(e.customerId)||{}}catch(e){console.warn(`Error fetching customer for memo:`,e)}let n=Number(e.bill)||0,r=Number(e.paid)||0,i=e.prevDue===void 0?Number(t.totalDue||0)-(n-r):Number(e.prevDue),o=e.currentDue===void 0?P(i+(n-r)):Number(e.currentDue);return{...e,customerName:e.customerName||t.name||`Unknown Customer`,customerPhone:t.phone||``,customerAccountNo:t.accountNo||``,customerAddress:t.address||``,customerZone:t.zone||``,customerTotalDue:t.totalDue||0,computedPrevDue:P(i),computedCurrentDue:P(o)}}async function ss(e=10){try{let t=await o.collection.orderBy(`createdAt`,`desc`).limit(e).get(),n=[];return t.forEach(e=>{let t=e.data();t.voucherNo&&t.voucherNo!==`OPENING`&&n.push({id:e.id,...t})}),n}catch(e){return console.error(`getRecentMemos error:`,e),[]}}function cs(e){if(!e)return``;let t=Number(e.bill)||0,n=Number(e.paid)||0,r=Number(e.computedPrevDue)||0,i=Number(e.computedCurrentDue)||0,a=String(e.customerName||`গ্রাহক`).replace(/^\[.*?\]\s*/,``).trim(),o=``;o=t>0&&n>=t?`<span class="px-2.5 py-1 rounded-full text-xs font-black bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"><i class="fa-solid fa-circle-check mr-1"></i>পরিশোধিত (PAID)</span>`:t>0&&n<t?`<span class="px-2.5 py-1 rounded-full text-xs font-black bg-red-500/10 text-red-400 border border-red-500/20"><i class="fa-solid fa-circle-exclamation mr-1"></i>বকেয়া (DUE)</span>`:n>0?`<span class="px-2.5 py-1 rounded-full text-xs font-black bg-purple-500/10 text-purple-400 border border-purple-500/20"><i class="fa-solid fa-receipt mr-1"></i>জমা রিসিট (COLLECTION)</span>`:`<span class="px-2.5 py-1 rounded-full text-xs font-black bg-blue-500/10 text-blue-400 border border-blue-500/20"><i class="fa-solid fa-info-circle mr-1"></i>এন্ট্রি</span>`;let s=``;n>0&&e.receivedType&&(s=`<span class="px-2 py-0.5 rounded text-[10px] font-black bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 ml-1.5">${j(e.receivedType)}${e.receivedFrom?` • `+j(e.receivedFrom):``}</span>`);let c=``;return e.hasItems&&e.items&&e.items.length>0&&(c=`
        ...
        <div class="bg-slate-900/90 border border-slate-700/80 rounded-3xl p-4 sm:p-6 shadow-2xl backdrop-blur-xl animate-fade-in font-bn">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4 mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="text-xl font-black text-white tracking-tight">মেমো #${j(e.voucherNo||e.id.slice(-6).toUpperCase())}</span>
                            ${o}
                        </div>
                        <div class="flex items-center gap-2 text-xs text-slate-400 mt-0.5">
                            <span><i class="fa-regular fa-calendar text-blue-400 mr-1"></i>${A(e.date)}</span>
                            <span>•</span>
                            <span><i class="fa-solid fa-user-pen text-slate-500 mr-1"></i>${j(e.createdBy||`System`)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <button onclick="window.printReceiptEngine('${e.id}', 'a4')" class="h-9 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs flex items-center gap-2 transition-all shadow-lg shadow-blue-600/20 active:scale-95 cursor-pointer" title="A4 সাইজ ইনভয়েস প্রিন্ট">
                        <i class="fa-solid fa-print"></i><span>A4 প্রিন্ট / PDF</span>
                    </button>
                    <button onclick="window.printReceiptEngine('${e.id}', 'pos')" class="h-9 px-3.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs flex items-center gap-2 border border-slate-700 transition-all active:scale-95 cursor-pointer" title="৮০মিমি POS থার্মাল প্রিন্ট">
                        <i class="fa-solid fa-receipt"></i><span>POS মেমো</span>
                    </button>
                    <button onclick="window.shareMemoOnWhatsApp('${e.id}')" class="h-9 px-3.5 rounded-xl bg-emerald-600/20 hover:bg-emerald-600 text-emerald-400 hover:text-white font-bold text-xs flex items-center gap-2 border border-emerald-500/30 transition-all active:scale-95 cursor-pointer" title="কাস্টমারকে WhatsApp মেসেজ পাঠান">
                        <i class="fa-brands fa-whatsapp text-sm"></i><span>WhatsApp</span>
                    </button>
                    <button onclick="window.viewCustomerLedgerDirectly('${e.customerId}', '${j(a)}', '${j(e.customerAccountNo||``)}')" class="h-9 px-3.5 rounded-xl bg-purple-600/20 hover:bg-purple-600 text-purple-300 hover:text-white font-bold text-xs flex items-center gap-2 border border-purple-500/30 transition-all active:scale-95 cursor-pointer" title="কাস্টমারের সম্পূর্ণ খতিয়ান দেখুন">
            ...
            <div class="bg-slate-950/60 border border-slate-800 rounded-2xl p-4 mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    <div>
                        ...
                        <div class="text-sm font-black text-white truncate">${j(a)}</div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="text-sm font-bold text-slate-300 truncate">${j(e.customerZone||e.customerAddress||`-`)}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                ...
                <div class="bg-slate-950/70 border border-slate-800 rounded-2xl p-4 space-y-2 text-xs">
                    ...
                    <div class="flex justify-between text-slate-300">
                        ...
                        <span class="font-mono font-bold">৳${k(r)}</span>
                    </div>
                    <div class="flex justify-between text-red-400 font-bold">
                        <span>আজকের বিল:</span>
                        <span class="font-mono font-black">৳${k(t)}</span>
                    </div>
                    <div class="flex justify-between text-emerald-400 font-bold items-center">
                        <span>আজকের জমা:${s}</span>
                        <span class="font-mono font-black">- ৳${k(n)}</span>
                    </div>
                    <div class="pt-2 border-t border-slate-800 flex justify-between items-center text-sm font-black ${i>0?`text-red-400`:`text-emerald-400`}">
                        <span>বর্তমান মোট বকেয়া:</span>
                        <span class="font-mono text-base bg-slate-900 px-3 py-1 rounded-xl border border-slate-800">
                            ৳${k(Math.abs(i))} ${i<0?`(অ্যাডভান্স)`:``}
                        </span>
                    </div>
                </div>
            </div>
        </div>
    `}window.shareMemoOnWhatsApp=async function(e){try{let{TransactionDAO:t,SettingsDAO:n}=await v(async()=>{let{TransactionDAO:e,SettingsDAO:t}=await import(`./dao-BEt4bkmS.js`).then(e=>e.d);return{TransactionDAO:e,SettingsDAO:t}},__vite__mapDeps([2,1,3,4,5])),r=await t.getById(e);if(!r)return;let i=r.customerPhone||``,a=r.customerName||`কাস্টমার`,o=A(r.date),s=Number(r.bill)||0,c=Number(r.paid)||0,l=Number(r.currentDue)||0,u=l<0?`অ্যাডভান্স জমা: ৳ ${k(Math.abs(l))}`:`বর্তমান মোট বকেয়া: ৳ ${k(l)}`,d=`${window.location.origin}${window.location.pathname}?view=public-memo&id=${e}`,f=`আসসালামু আলাইকুম ${a},\nমেসার্স মা মোটরস্ থেকে আপনার মেমো তথ্য:\n\n${r.voucherNo?`মেমো #${r.voucherNo}\n`:``}তারিখ: ${o}\nআজকের বিল: ৳ ${k(s)}\nআজকের জমা: ৳ ${k(c)}\n---------------------------------\n${u}\n\nআপনার ডিজিটাল মেমোর PDF দেখতে নিচের লিংকে ক্লিক করুন:\n${d}\n\nধন্যবাদ! — মেসার্স মা মোটরস্`;if(window.sendWhatsApp)window.sendWhatsApp(i,f);else{let e=i.replace(/[^0-9]/g,``),t=e.startsWith(`88`)?e:`88`+e;window.open(`https://api.whatsapp.com/send?phone=${t}&text=${encodeURIComponent(f)}`,`_blank`)}}catch(e){console.error(`WhatsApp share error:`,e)}},window.viewCustomerLedgerDirectly=function(e,t,n){e&&ws(`statement`,{customerId:e,customerName:t,accountNo:n})};var ls;async function us(e,t={}){e&&(e.innerHTML=`
    ...
    `,ds(t),ps())}function ds(e={}){let t=document.getElementById(`memo-search-input`),n=document.getElementById(`memo-search-clear-btn`);t&&(setTimeout(()=>{t.focus()},100),t.oninput=e=>{let t=e.target.value;if(n&&n.classList.toggle(`hidden`,t.length===0),clearTimeout(ls),!t.trim()){ms();return}ls=setTimeout(()=>{fs(t)},200)},t.onkeydown=e=>{e.key===`Enter`&&(e.preventDefault(),clearTimeout(ls),fs(t.value))},e.memoNo&&(t.value=e.memoNo,n&&n.classList.remove(`hidden`),fs(e.memoNo)))}async function fs(e){let t=document.getElementById(`memo-search-result-area`),n=document.getElementById(`memo-multiple-matches`);if(t){if(!e||!e.trim()){ms();return}t.innerHTML=`
    ...
    `;try{let r=await as(e);if(!r||r.length===0){n&&n.classList.add(`hidden`),t.innerHTML=`
            ...
            `;return}r.length>1&&n?(n.classList.remove(`hidden`),n.innerHTML=`
                <div class="bg-slate-900/60 border border-slate-800 rounded-2xl p-3 mb-4">
                    <div class="text-[11px] font-black text-cyan-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                        <i class="fa-solid fa-layer-group"></i> <span>একাধিক মেমো পাওয়া গেছে (${r.length}টি) - পছন্দ করুন:</span>
                    </div>
            ...
            `,window._currentMemoSearchResults=r):n&&n.classList.add(`hidden`),t.innerHTML=cs(r[0])}catch(e){console.error(`executeMemoSearch UI error:`,e),t.innerHTML=`
            <div class="p-8 text-center text-red-400 font-bold bg-slate-900/60 rounded-3xl border border-red-500/20">
                মেমো অনুসন্ধানে সমস্যা হয়েছে: ${j(e.message||`Error`)}
            </div>
        `}}}window.selectSpecificMemoMatch=function(e){if(window._currentMemoSearchResults&&window._currentMemoSearchResults[e]){let t=window._currentMemoSearchResults[e],n=document.getElementById(`memo-search-result-area`);n&&(n.innerHTML=cs(t)),document.querySelectorAll(`.memo-match-btn`).forEach(t=>{t.className=`memo-match-btn px-3 py-1.5 rounded-xl ${Number(t.dataset.idx)===e?`bg-cyan-600 text-white font-black`:`bg-slate-800 hover:bg-slate-700 text-slate-300`} text-xs flex items-center gap-2 border border-slate-700 transition-all cursor-pointer`})}};async function ps(){let e=document.getElementById(`memo-recent-chips`);if(e)try{let t=await ss(8);if(t.length===0){e.innerHTML=`<span class="text-slate-600 text-xs">কোনো মেমো পাওয়া যায়নি</span>`;return}e.innerHTML=t.map(e=>`
        ...
        `).join(``)}catch(e){console.error(`loadRecentMemoChips error:`,e)}}window.searchMemoDirectly=function(e){let t=document.getElementById(`memo-search-input`),n=document.getElementById(`memo-search-clear-btn`);t&&(t.value=e,n&&n.classList.remove(`hidden`),fs(e))},window.clearMemoSearchInput=function(){let e=document.getElementById(`memo-search-input`),t=document.getElementById(`memo-search-clear-btn`);e&&(e.value=``,e.focus(),t&&t.classList.add(`hidden`),ms())};function ms(){let e=document.getElementById(`memo-search-result-area`),t=document.getElementById(`memo-multiple-matches`);t&&t.classList.add(`hidden`),e&&(e.innerHTML=`
            <div class="bg-slate-900/40 border border-dashed border-slate-800 rounded-3xl p-12 text-center text-slate-500 space-y-3">
                <div class="w-16 h-16 rounded-3xl bg-slate-800/40 border border-slate-700/40 flex items-center justify-center mx-auto text-slate-600 text-2xl">
                    <i class="fa-solid fa-barcode"></i>
                </div>
...
_মা মোটরস্ ইআরপি সিস্টেম থেকে প্রস্তুতকৃত_`}async function rc(e,t,n){if(!n){R.default.fire({title:`ডাটা লোড হয়নি`,text:`অনুগ্রহ করে আগে লেজার লোড করুন`,icon:`warning`,customClass:{popup:`!bg-slate-900 !text-white !rounded-3xl border border-slate-700`}});return}let r=nc(e,t,document.getElementById(`bl-from-date`)?.value||``,document.getElementById(`bl-to-date`)?.value||``,n),{value:i}=await R.default.fire({title:`<div class="flex items-center justify-center gap-2 text-emerald-400 font-bn text-lg font-black">
    ...
    `,await gc()}async function gc(){try{let[e,t]=await Promise.all([f.getActiveBanks(),l.getActiveCollectors()]);$=[...e.map(e=>({...e,isCash:!1})),...t.map(e=>({...e,isCash:!0}))];let n=await Promise.all($.map(e=>Ro(e.name,e.isCash)));$.forEach((e,t)=>{e.balance=n[t]}),_c(),await vc(`month`)}catch(e){console.error(`Error loading accounts:`,e);let t=document.getElementById(`banking-accounts-grid`);t&&(t.innerHTML=`<div class="text-center py-12 col-span-full text-red-400 font-bold">ব্যালেন্স লোড করতে সমস্যা হয়েছে!</div>`)}}function _c(){let e=document.getElementById(`banking-accounts-grid`);if(!e)return;let t=$.filter(e=>!e.isCash).length,n=$.filter(e=>e.isCash).length,r=document.getElementById(`count-all`),i=document.getElementById(`count-bank`),a=document.getElementById(`count-cash`);r&&(r.innerText=$.length),i&&(i.innerText=t),a&&(a.innerText=n);let o=mc.toLowerCase().trim(),s=$.filter(e=>{let t=pc===`all`?!0:pc===`cash`?e.isCash:!e.isCash,n=!o||e.name.toLowerCase().includes(o);return t&&n});if(s.length===0){e.innerHTML=`<div class="col-span-full text-center py-12 text-slate-500 font-bold italic">কোনো অ্যাকাউন্ট পাওয়া যায়নি।</div>`;return}e.innerHTML=s.map(e=>{let t=e.balance||0,n=t<0,r=e.isCash,i=r?`<i class="fa-solid fa-wallet text-emerald-400 text-xl"></i>`:`<i class="fa-solid fa-building-columns text-blue-400 text-xl"></i>`,a=r?`bg-emerald-500/10 border-emerald-500/20`:`bg-blue-500/10 border-blue-500/20`,o=r?`<span class="px-2 py-0.5 rounded-md text-[10px] font-black bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase tracking-wider">CASH BOX</span>`:`<span class="px-2 py-0.5 rounded-md text-[10px] font-black bg-blue-500/10 text-blue-400 border border-blue-500/20 uppercase tracking-wider">BANK</span>`,s=n?`<span class="px-2 py-0.5 rounded-md text-[10px] font-black bg-red-500/20 text-red-400 border border-red-500/40 animate-pulse flex items-center gap-1"><i class="fa-solid fa-triangle-exclamation text-[9px]"></i> ঘাটতি</span>`:``,c=n?`border-red-500/50 bg-gradient-to-br from-red-950/20 to-slate-900/90 shadow-red-950/20`:`border-slate-800/90 bg-slate-900/80 hover:border-purple-500/50 hover:shadow-purple-950/20`,l=e.name.replace(/'/g,`\\'`),u=Math.abs(t)>0?L(Math.abs(t)):`শূন্য টাকা`,d=n?`(ঘাটতি: ${u})`:`(${u})`;return`
        ...
        `}).join(``)}async function vc(e=`month`){let t=document.getElementById(`banking-dashboard-container`);if(t)try{let n=await tc(e),r=$.filter(e=>!e.isCash).reduce((e,t)=>e+(t.balance||0),0),i=$.filter(e=>e.isCash).reduce((e,t)=>e+(t.balance||0),0),a=r+i;t.innerHTML=`

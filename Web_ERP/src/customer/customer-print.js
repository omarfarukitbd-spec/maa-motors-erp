...
export async function printFilteredCustomerList() {
    ...
    const rowsArray = filtered.map((c, idx) => {
        ...
        let openDateDisp = '-', openTimeDisp = '', openDayDisp = '';
        if (c.createdAt) {
            try {
                ...
                if (!isNaN(dateObj.getTime())) {
                    ...
                    openDayDisp = getDayOfWeekBangla(dateObj);
                }
            } catch (e) {
        ...
        if (openDateDisp === '-' && (c.openingDate || c.date)) {
            ...
            openDayDisp = getDayOfWeekBangla(c.openingDate || c.date);
        }
        ...
        if (selectedCols.date) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 4px 2px; font-size: 10px; font-family: 'Inter', sans-serif; color: #334155; line-height: 1.15; white-space: nowrap;"><div style="font-weight: 700;">${openDateDisp}</div><div style="font-size: 8px; color: #64748b; font-family: 'Hind Siliguri', sans-serif; font-weight: 600; margin-top: 1px;">${openDayDisp}${openTimeDisp ? ` • ${openTimeDisp}` : ''}</div></td>`;

...
export async function printFilteredCustomerList() {
    ...
    const rowsArray = filtered.map((c, idx) => {
        ...
        // Format phone with line breaks if multiple numbers exist
        ...
        let cellsHtml = '';
        if (selectedCols.sl) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 2px; font-size: 9.5px; font-family: 'Inter', sans-serif; color: #475569;">${idx + 1}</td>`;
        if (selectedCols.date) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3px 2px; font-size: 9px; font-family: 'Inter', sans-serif; color: #334155; line-height: 1.15; white-space: nowrap;"><div style="font-weight: 700;">${openDateDisp}</div>${openTimeDisp ? `<div style="font-size: 8px; color: #64748b; font-weight: 500; margin-top: 1px;">${openTimeDisp}</div>` : ''}</td>`;
        if (selectedCols.acc) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 2px; font-size: 10.5px; font-weight: 800; font-family: 'Inter', monospace; color: #0284c7; white-space: nowrap;">${escapeHTML(c.accountNo || '-')}</td>`;
        if (selectedCols.code) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 2px; font-size: 10px; font-weight: 700; font-family: 'Inter', monospace; color: #475569; white-space: nowrap;">${escapeHTML(zCode)}</td>`;
        if (selectedCols.name) cellsHtml += `<td style="text-align:left; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 5px; font-size: 10px; font-family: 'Hind Siliguri', 'Kalpurush', sans-serif; line-height: 1.2; color: #0f172a; word-break: break-word;"><strong>${escapeHTML(c.name)}</strong></td>`;
    ...
    let headerThHtml = '';
    if (selectedCols.sl) headerThHtml += `<th style="text-align: center; border: 1px solid #cbd5e1; padding: 5px 2px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">SL</th>`;
    if (selectedCols.date) headerThHtml += `<th style="text-align: center; border: 1px solid #cbd5e1; padding: 5px 2px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">তারিখ</th>`;
    if (selectedCols.acc) headerThHtml += `<th style="text-align: center; border: 1px solid #cbd5e1; padding: 5px 2px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">A/C NO</th>`;
    if (selectedCols.code) headerThHtml += `<th style="text-align: center; border: 1px solid #cbd5e1; padding: 5px 2px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">কোড</th>`;
    if (selectedCols.name) headerThHtml += `<th style="text-align: left; border: 1px solid #cbd5e1; padding: 5px 5px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">কাস্টমারের নাম</th>`;
    if (selectedCols.addr) headerThHtml += `<th style="text-align: left; border: 1px solid #cbd5e1; padding: 5px 5px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">ঠিকানা</th>`;
    if (selectedCols.phone) headerThHtml += `<th style="text-align: center; border: 1px solid #cbd5e1; padding: 5px 2px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">মোবাইল নম্বর</th>`;
    if (selectedCols.zone) headerThHtml += `<th style="text-align: center; border: 1px solid #cbd5e1; padding: 5px 2px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">জোন</th>`;
    if (selectedCols.bal) headerThHtml += `<th style="text-align: right; border: 1px solid #cbd5e1; padding: 5px 4px; font-size: 9.5px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">ব্যালেন্স (৳)</th>`;

...
export async function printFilteredCustomerList() {
    ...
    const rowsArray = filtered.map((c, idx) => {
        ...
        let cellsHtml = '';
        if (selectedCols.sl) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 3px; font-size: 10px; font-family: 'Inter', sans-serif; color: #475569;">${idx + 1}</td>`;
        if (selectedCols.date) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 2px; font-size: 9.5px; font-family: 'Inter', sans-serif; color: #334155; line-height: 1.15; white-space: nowrap;"><div style="font-weight: 700;">${openDateDisp}</div>${openTimeDisp ? `<div style="font-size: 8px; color: #64748b; font-weight: 500; margin-top: 1px;">${openTimeDisp}</div>` : ''}</td>`;
        if (selectedCols.acc) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 3px; font-size: 10.5px; font-weight: 800; font-family: 'Inter', monospace; color: #0284c7;">${escapeHTML(c.accountNo || '-')}</td>`;
        if (selectedCols.code) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 3px; font-size: 10px; font-weight: 700; font-family: 'Inter', monospace; color: #475569;">${escapeHTML(zCode)}</td>`;
        if (selectedCols.name) cellsHtml += `<td style="text-align:left; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 5px; font-size: 10.5px; font-family: 'Kalpurush', 'Hind Siliguri', sans-serif; line-height: 1.15; color: #0f172a;"><strong>${escapeHTML(c.name)}</strong></td>`;
        if (selectedCols.addr) cellsHtml += `<td style="text-align:left; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 5px; font-size: 10px; font-family: 'Kalpurush', 'Hind Siliguri', sans-serif; line-height: 1.15; color: #334155;">${escapeHTML(c.address || '-')}</td>`;
        if (selectedCols.phone) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 3px; font-size: 10px; font-family: 'Inter', sans-serif; white-space: nowrap; color: #334155;">${escapeHTML(c.phone || '-')}</td>`;
        if (selectedCols.zone) cellsHtml += `<td style="text-align:center; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 3px; font-size: 10px; font-family: 'Kalpurush', 'Hind Siliguri', sans-serif;">${zoneBadge}</td>`;
        if (selectedCols.bal) cellsHtml += `<td style="text-align:right; vertical-align:middle; border: 1px solid #e2e8f0; padding: 3.5px 5px; font-size: 10.5px; font-weight: 900; color: ${dueColor}; font-family: 'Inter', sans-serif; white-space: nowrap;">${dueDisp}</td>`;
    ...
    let headerThHtml = '';
    if (selectedCols.sl) headerThHtml += `<th style="width: 32px; text-align: center; border: 1px solid #cbd5e1; padding: 5px 3px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">SL</th>`;
    if (selectedCols.date) headerThHtml += `<th style="width: 70px; text-align: center; border: 1px solid #cbd5e1; padding: 5px 3px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">তারিখ</th>`;
    if (selectedCols.acc) headerThHtml += `<th style="width: 65px; text-align: center; border: 1px solid #cbd5e1; padding: 5px 3px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">A/C NO</th>`;
    if (selectedCols.code) headerThHtml += `<th style="width: 50px; text-align: center; border: 1px solid #cbd5e1; padding: 5px 3px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">কোড</th>`;
    if (selectedCols.name) headerThHtml += `<th style="text-align: left; border: 1px solid #cbd5e1; padding: 5px 5px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">কাস্টমারের নাম</th>`;
    if (selectedCols.addr) headerThHtml += `<th style="text-align: left; border: 1px solid #cbd5e1; padding: 5px 5px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">ঠিকানা</th>`;
    if (selectedCols.phone) headerThHtml += `<th style="width: 100px; text-align: center; border: 1px solid #cbd5e1; padding: 5px 3px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">মোবাইল নম্বর</th>`;
    if (selectedCols.zone) headerThHtml += `<th style="width: 65px; text-align: center; border: 1px solid #cbd5e1; padding: 5px 3px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">জোন</th>`;
    if (selectedCols.bal) headerThHtml += `<th style="width: 85px; text-align: right; border: 1px solid #cbd5e1; padding: 5px 5px; font-size: 10px; font-weight: 900; color: #1e293b; font-family: 'Hind Siliguri', sans-serif;">ব্যালেন্স (৳)</th>`;
        ...
        <div style="display: flex; justify-content: flex-end; margin-top: 10px; page-break-inside: avoid; break-inside: avoid;">
            <div style="width: 260px; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 8px; padding: 8px 10px; font-family: 'Hind Siliguri', sans-serif; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                ...
                <div style="display: flex; justify-content: space-between; font-size: 11px;">
                    ...
                    <strong style="color: #dc2626; font-size: 13.5px; font-weight: 900;">৳ ${formatAmountWithComma(totalDue)}</strong>
                </div>
            </div>
        </div>
    `;

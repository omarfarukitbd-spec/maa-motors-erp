...
export function printLedger() {
    ...
    const html = `
        <html><head><title>${currentAccountName} Ledger</title>
        <style>body { font-family: Arial, sans-serif; padding: 20px; } table { width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 12px; } th { background: #f0f0f0; padding: 10px; text-align: left; border-bottom: 2px solid #333; } .text-right { text-align: right; }</style>
            ...
            <h2 style="text-align: center;">Maa Motors ERP</h2><h3 style="text-align: center;">Bank Ledger: ${currentAccountName}</h3>
            ...
            <table><thead><tr><th>Date</th><th>Description / Note</th><th class="text-right">Deposit</th><th class="text-right">Withdrawal</th><th class="text-right">Balance</th></tr></thead><tbody>${rowsHtml}</tbody></table>
...
export function exportLedgerExcel() {
    ...
    currentLedgerData.transactions.forEach(t => {
        ...
        rows.push([formattedDate, desc, t.isCredit ? t.amount : 0, t.isDebit ? t.amount : 0, t.runningBalance]);
    });

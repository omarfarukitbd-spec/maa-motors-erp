...
export async function renderDashboardView(container, session) {
    previousViewName = 'dashboard';

    // Load Settings
    try {
        const sSnap = await db.collection('settings').doc('app_settings').get();
        if (sSnap.exists) appSettings = sSnap.data();
    } catch(e) {}
    ...
    unsubscribeCustomers = db.collection('customers').onSnapshot(snap => {
        allCustomersCache = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));

        if (session.role === 'Customer' && session.customerId) {
            const customer = allCustomersCache.find(c => c.id === session.customerId);
            if (customer) {
...
function renderWelcomeBossCard() {
...
export async function renderTodayCollectionView(container) {
    ...
    unsubscribeToday = db.collection('transactions')
        .where('date', '==', todayStr)
        .onSnapshot(snap => {
...
function renderDirectoryCardsHtml(customers) {
...
export async function loadAndRenderCustomerLedger(container, customer) {
    ...
    try {
        ...
        customerTransactions = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }))
            .sort((a, b) => new Date(b.date) - new Date(a.date));

        Swal.close();
        renderCustomerLedgerView(container, customer, customerTransactions);
    } catch(e) {
        console.error(e);
...
function renderTxnCardsHtml(txns) {
    if (txns.length === 0) {

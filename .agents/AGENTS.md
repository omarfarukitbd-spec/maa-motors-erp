...
8. **Modular Architecture & Flexible 300-Line Guideline (with Critical Need Exception)**:
   - **Target Guideline (~300 Lines)**: Maintain clean, focused modules by keeping `.js` files around 300 lines as a general best practice.
   - **Critical Need Exception (300+ Lines Permitted)**: If a feature, complex workflow, tight logic coupling, or critical bug fix strictly requires a file to exceed 300 lines (e.g., up to 500 lines), IT IS FULLY PERMITTED. Never fragment code or create unnecessary micro-files if keeping it together is safer and more cohesive.
   - **Absolute Ban on Code Deletion / Dangerous Minification**: NEVER delete, drop, or dangerously squeeze working logic, comments, or error handlers merely to artificially stay below 300 lines. Correctness, financial safety, and code readability ALWAYS take precedence over line counts.
   - **Dedicated Feature Folder**: When a module naturally grows large and can be cleanly separated without risk, split into focused sub-modules (UI, calculation, actions, and barrel entry).

...
filepath = r'd:\Office Excel\Business_Ledger_Pro.xlsx'
...
# 1. Locked Formats (For formulas) - Locked by default
locked_format = workbook.add_format({'border': 1, 'valign': 'vcenter', 'bg_color': '#ECF0F1', 'bold': True, 'locked': True})
locked_currency = workbook.add_format({'border': 1, 'valign': 'vcenter', 'num_format': '#,##0.00', 'bg_color': '#ECF0F1', 'bold': True, 'locked': True})
link_format = workbook.add_format({
    'font_color': 'blue', 'underline': True, 'border': 1, 
    'align': 'center', 'valign': 'vcenter', 'bold': True, 'bg_color': '#D6EAF8', 'locked': True
})

# 2. Unlocked Formats (For manual entry)
...
unlocked_currency = workbook.add_format({'border': 1, 'valign': 'vcenter', 'num_format': '#,##0.00', 'locked': False})

# Colored Unlocked Formats for Debit/Credit
debit_format = workbook.add_format({'border': 1, 'valign': 'vcenter', 'num_format': '#,##0.00', 'bg_color': '#FDEDEC', 'locked': False}) # Light Red
credit_format = workbook.add_format({'border': 1, 'valign': 'vcenter', 'num_format': '#,##0.00', 'bg_color': '#EAFAF1', 'locked': False}) # Light Green
...
# Dashboard formats (All locked)
dash_title = workbook.add_format({'bold': True, 'font_size': 20, 'align': 'center', 'bg_color': '#34495E', 'font_color': 'white', 'locked': True})
...
ws_dash.protect('451060') # Protect entire sheet
...
ws_customers.freeze_panes(1, 0) # Freeze top row
ws_customers.protect('451060')
...
ws_customers.write(1, 0, 'C-001', unlocked_text)
ws_customers.write(1, 1, 'তপু ভাই, আল রাফি মোটরস', unlocked_text)
ws_customers.write(1, 2, '01720643322', unlocked_text) 
ws_customers.write(1, 3, 'ধোলাইখাল, ঢাকা', unlocked_text)

ws_customers.write(2, 0, 'C-002', unlocked_text)
ws_customers.write(2, 1, 'জামাল, রহমান টাওয়ার', unlocked_text)
ws_customers.write(2, 2, '01676218863', unlocked_text)
ws_customers.write(2, 3, '', unlocked_text)

# ==========================================
# 3. Ledger Sheet
# ==========================================
ws_ledger = workbook.add_worksheet('খতিয়ান')
ws_ledger.freeze_panes(1, 0) # Freeze top row
ws_ledger.protect('451060') # Protect sheet
...
ws_ledger.add_table('A1:H1005', {
    ...
    'columns': [
        ...
        {'header': headers_ledger[7]}
    ],
    'style': 'Table Style Light 1'
})

for row in range(1, 1004): # Reduced by 1 to accommodate total row at the end 
    ...
    ws_ledger.write_formula(row, 2, formula_id, locked_format)
    ...
    ws_ledger.write_formula(row, 7, formula_balance, locked_format)
...
ws_ledger.write(3, 0, '02/11/2022', unlocked_date)
ws_ledger.write(3, 1, 'তপু ভাই, আল রাফি মোটরস', unlocked_text)
ws_ledger.write(3, 3, 'Received এমদাদ', unlocked_text)
ws_ledger.write(3, 6, 240600, credit_format)

workbook.close()
print("Business_Ledger_Pro generated successfully!")
